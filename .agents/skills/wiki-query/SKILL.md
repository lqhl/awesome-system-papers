...
在生成对话答案、表格、延伸阅读摘要或存档页前，必须完整阅读并执行 [中文写作与术语解释契约](../_shared/chinese-writing.md)。

同时执行契约中的“去除模板化和 AI 味”自审，删除空泛归因和套话，让判断落到具体机制、证据或边界。常用系统缩写直接使用；短答案只解释非标准术语，长篇跨领域答案确有需要时才增加简短的「阅读提示」。
