...
![](images/d92401b1c1b11eefb379e41a3a8d9dc33d92f3e773d1599f41d8e901b22590e8.jpg)  
...
We implement the NI and router modules, as well as a full cluster tile, in TSMC 7 nm technology using FUSION COMPILER 2024.09, with a 1 GHz frequency target under worst-case conditions $( \mathrm { S S } , - 4 0 ^ { \circ } \mathrm { C } , 0 . 6 7 5 \mathrm { V } )$ , observing no timing degradations.
...
![](images/d3b228f492ea473f7d6299a0247c90826a6edb81cd8747f943a174b6f0754c10.jpg)  
(a)

![](images/f6af06ffb9c3f51b7c73f26678d1b71a8bfcb1790335dc1d872d5c0132185e7f.jpg)  
(b)  
...
![](images/eb97f6f1e71dd81ae352988c45fd34bd1ae96bd1e45950ce3309fad2fefbc3cd.jpg)  
...
![](images/65f9c196fc8b56c0b6b3238cb6915b9582fb5ae37e37d06792b492d1e05f2a0a.jpg)  
...
$$
T _ { n a i v e } = \sum _ { i = 1 } ^ { c } ( \alpha _ { i } + \beta n + \delta ) - \delta\tag{1}
$$

where $\alpha _ { i }$ is the round-trip latency of the DMA transfer in each iteration $i , \beta$ the inverse bandwidth (cycles per beat), n the transfer size (in beats) and δ the synchronization time. Figure 4b illustrates an optimized implementation (seq) dividing the transfer into k batches that are pipelined across clusters. The runtime then becomes:

$$
T _ { s e q } = \sum _ { i = 1 } ^ { k + c - 1 } ( \alpha _ { i } + \frac { n } { k } \beta + \delta ) - \delta\tag{2}
$$

As this equation sugges $^ { \mathrm { { . } } }$ an optimal batch size exists that minimizes overall transmission time. Intuitively, this is due to the additional round-trip latency (α) and synchronization (δ) overheads offsetting the benefit from the decreased batch size $\frac { n } { k }$ . Our second baseline (tree) implements a binarytree multicast, as depicted in Figure 4c, modeled as:

![](images/92e0f1086faaef91e0171c356e75fe69604cffac1b398559a4d9ce7c09140de4.jpg)  
Figure 5. Runtime (in cycles) of: (a) a 1D multicast transfer; (b) the seq implementation for various settings of $\alpha _ { i } + \delta , \forall i > 0$ labeled next to each curve; (c) a 2D multicast transfer.

$$
T _ { t r e e } = \sum _ { i = 0 } ^ { \log _ { 2 } c } ( \alpha _ { i } + n \beta + \delta ) - 2 \delta\tag{3}
$$
...
$$
\begin{array} { l } { { T _ { s e q } = t _ { m } + 2 ( c - 2 ) \operatorname* { m a x } ( t _ { m } , t _ { c } ) + k t _ { c } + } } \\ { { \phantom { T _ { s e q } = } + ( 2 ( c - 2 ) + k ) \delta } } \end{array}\tag{5}
$$

$$
\begin{array} { r l } & { T _ { t r e e } = \left\{ t _ { m } + \delta + \left( k - 1 \right) \left[ \operatorname* { m a x } ( t _ { m } , t _ { c } ) + \delta \right] + \right. } \\ & { \qquad \left. + t _ { c } \right\} * \log _ { 2 } c } \end{array}\tag{6}
$$

![](images/2de521f517a927aa11f806a7269be5803e695285a969b7b041eba8fc6dcc0f4f.jpg)  
...
with $\begin{array} { r } { t _ { m } = \alpha _ { m } + \frac { n } { k } \beta _ { m } } \end{array}$ and $\begin{array} { r } { t _ { c } = \alpha _ { c } + \frac { n } { k } \beta _ { c } } \end{array}$ , given $\alpha _ { m }$ and $\beta _ { m }$ the round-trip latency and inverse bandwidth of the DMA transfers, and $\alpha _ { c }$ and $\beta _ { c }$ respectively a constant instruction overhead and the inverse throughput (cycles per beat) of the computation. As seen for multicast, tiling the transfers introduces additional synchronization and round-trip latency overheads, leading to the existence of an optimal tile size which minimizes the transmission time.

Figure 7a presents the comparison to hardware-accelerated reduction. Again, our models accurately reflect the measured runtimes. The hardware implementation consistently outperforms both baselines, achieving speedups between 2.0× and 3.0× over the best software baseline, ${ { T } _ { s w } } \ =$ $\operatorname* { m i n } ( T _ { s e q } , T _ { t r e e } )$ , for transfer sizes between 1 and 32 KiB.

Figure 6 and Equations (5)–(6) can be easily generalized to a reduction involving multiple rows, as presented in Section B.2. In short, a 2D reduction can be implemented in two steps: 1) c independent 1D reduction operations combine the data across each row, in parallel, and 2) a final column-wise 1D reduction combines these partial results.
...
![](images/1e05c18be8cbdcc282e37ca36176bc185ae4bea56a6223abddc68b80f17c3299.jpg)  
...
Consider a GEMM operation $C = \alpha A \times B + \beta C$ , with matrices A, B and C respectively of size $M \times K , K \times N$ and $M \times N$ We map the GEMM computation on the system described in Sections 2.2 and 3.2, employing the SUMMA dataflow (van de Geijn & Watts, 1995), with double-buffering to overlap computation and communication, as illustrated in Figure 8a. In every iteration, each cluster computes a subproblem of size $M _ { t } \times N _ { t } \times K _ { t }$ . To benefit from data reuse at the L2 memory level, we assume that the memory tiles are dimensioned to jointly fit a maximum problem size $M \times N \times K$ that satisfies either one of the following conditions: $M \gg r M _ { t } , N \gg c N _ { t }$ and $K \gg K _ { t }$ . For the sake of example, we assume $K \gg K _ { t }$

![](images/f2bc231d291135ef8c5900e35cb4439d48b2a9caba7735cd591da53faf1c3d2a.jpg)  
...
$M = r M _ { t }$ and $N = c N _ { t }$ , but analogous arguments can be made for other configurations.

Under the $K \gg K _ { t }$ assumption, we can ignore boundary iterations and focus on a steady-state iteration’s runtime:

$$
T = \operatorname* { m a x } ( T _ { c o m p } , T _ { c o m m } )\tag{7}
$$

$$
T _ { c o m p } = \frac { 2 M _ { t } N _ { t } K _ { t } } { U t i l \cdot P e a k P e r f }\tag{8}
$$

$$
T _ { c o m m } = T _ { m c a s t _ { A } } + T _ { m c a s t _ { B } }\tag{9}
$$

where $T _ { m c a s t _ { A } }$ and $T _ { m c a s t _ { B } }$ are the times required to transfer a submatrix $A _ { i , * }$ to all clusters in row i and a submatrix $B _ { * , j }$ to all clusters in column j, respectively.
...
![](images/7869c30f83f8c5b9ff7c8e9e2010aa4822bb964592a1ab9f467d22b09805f2f2.jpg)  
...
<table><tr><td>Primitives</td><td>DMA Load</td><td>DMA Store</td><td>Hop</td><td> SPM Write</td><td>GEMM</td><td> SW Reduce</td><td>DCA Reduce</td></tr><tr><td>Energy</td><td> $2 . 2 \mathrm { p J } / \mathrm { B }$ </td><td> $2 . 4 \mathrm { p J / B }$ </td><td>1.1 pJ/B</td><td> $1 . 8 \mathrm { p J / B }$ </td><td>24.6pJ/OP</td><td> $2 2 . 4 \mathrm { p J / O P }$ </td><td>19.0pJ/OP0</td></tr><tr><td>SUMMA SW</td><td>66</td><td>983</td><td>1114</td><td>983</td><td>1049</td><td>0</td><td>0</td></tr><tr><td> SUMMA HW</td><td>66</td><td>660</td><td>983</td><td>983</td><td>1049</td><td>0</td><td>0</td></tr><tr><td>FCL SW</td><td>524</td><td>524</td><td>4524</td><td>522</td><td>1049</td><td>65</td><td>0</td></tr><tr><td>FCL HW</td><td>524</td><td>720</td><td>3932</td><td>350</td><td>1049</td><td>0</td><td>65</td></tr></table>
...
We measure the energy of the primitive operations reported in Table 1. We then break down each GEMM workload into its constituent operations, and count the number of occurrences of each primitive across all cluster tiles (reported in Table 1 for a 16×16 mesh), to obtain an estimate of the total energy. The software-based implementation always assumes the fastest software collective as in Section 4.3.

Figure 10a reports the energy savings on the SUMMA GEMM kernel across different mesh sizes. As quantified in Table 1 1 , hardware multicast reduces the number of DMA operations involved in the multicast transfers of $A _ { i , * }$ and $B _ { * , j }$ submatrices. While the total energy consumption remains dominated by computation, the communication savings grow with the mesh size, reaching overall energy savings of up to 1.17× for a 256 × 256 mesh, and demonstrating the increasing benefit of multicast support at scale.
...
![](images/8adb6897c861e865cf81965c5cb28e585ce300240f0d37b6d00b946c0a9a398e.jpg)  
...
Heißwolf, J., Weichslgartner, A., Zaib, A., König, R., Wild, T., Herkersdorf, A., Teich, J., and Becker, J. Hardware

supported adaptive data collection for Networks on Chip. In 2013 IEEE International Symposium on Parallel & Distributed Processing, Workshops and Phd Forum, pp. 153–162, 2013.

Hu, W., Lu, Z., Jantsch, A., and Liu, H. Power-efficient treebased multicast support for Networks-on-Chip. In 16th Asia and South Pacific Design Automation Conference (ASP-DAC 2011), pp. 363–368, 2011.
...
Shen, X.-W., Ye, X.-C., Tan, X., Wang, D., Zhang, L., Li, W.-M., Zhang, Z.-M., Fan, D.-R., and Sun, N.-H. An

efficient Network-on-Chip router for dataflow architecture. Journal of Computer Science and Technology, 32 (1):11–25, Jan 2017.
...
$$
T _ { n a i v e } = \sum _ { i = 1 } ^ { c + r - 1 } ( \alpha _ { i } + \beta n + \delta ) - \delta\tag{10}
$$

$$
T _ { s e q } = \sum _ { i = 1 } ^ { k + c + r - 2 } ( \alpha _ { i } + \frac { n } { k } \beta + \delta ) - \delta\tag{11}
$$

$$
T _ { t r e e } = \sum _ { i } ^ { \log _ { 2 } ( c r ) } ( \alpha _ { i } + n \beta + \delta ) - 2 \delta\tag{12}
$$

The same formula for the hw implementation:

$$
T _ { h w } = \alpha + ( n + c + r - 2 ) \beta\tag{13}
$$
...
$$
\begin{array} { l } { { \cal T } _ { t r e e } = \left\{ t _ { m } + \delta + \left( k - 1 \right) \left[ \operatorname* { m a x } ( t _ { m } , t _ { c } ) + \delta \right] + \right. } \\ { \left. ~ + t _ { c } \right\} * \left( \log _ { 2 } c + \log _ { 2 } r \right) } \end{array}\tag{14}
$$

<table><tr><td rowspan=1 colspan=1>Cluster 0</td><td rowspan=1 colspan=1>m0→co</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 1</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c0→c1</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 2</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c1→c2</td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 3</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c2→c3</td></tr><tr><td rowspan=1 colspan=2>Cluster 4</td><td rowspan=1 colspan=1>c0→c4</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 5</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c4→c5</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 6</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c5→c6</td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 7</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c6→c7</td></tr><tr><td rowspan=1 colspan=2>Cluster 8</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c4→c8</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 9</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c8→c9</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 10</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c9→c10</td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 11</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c10→c11</td></tr><tr><td rowspan=1 colspan=2>Cluster 12</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c8→c12</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 13</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c12→c13</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 14</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c13→c14</td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=2>Cluster 15</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c14→c15</td></tr></table>

Figure 11. 2D naive sequential multicast.  
![](images/366106b272b8726ccd7f7964be03b5a2d5d7221af3c667c79cdf713eae346fbf.jpg)  
Figure 12. 2D pipelined sequential multicast.

$$
\begin{array} { r l } & { T _ { s e q } = t _ { m } + 2 ( c - 2 ) \operatorname* { m a x } ( t _ { m } , t _ { c } ) + ( k - 1 ) t _ { c } + } \\ & { \qquad + \operatorname* { m a x } ( t _ { m } , t _ { c } ) + 2 ( r - 2 ) \operatorname* { m a x } ( t _ { m } , t _ { c } ) + } \\ & { \qquad + k t _ { c } + ( 2 ( c - 2 ) + 2 ( r - 2 ) + 2 k ) \delta } \end{array}\tag{15}
$$

<table><tr><td rowspan=2 colspan=1>Cluster 0</td><td rowspan=2 colspan=1>m0→co</td><td rowspan=2 colspan=1>c0→c8</td><td rowspan=2 colspan=1>c0→c4</td><td rowspan=2 colspan=1>c0→c2</td><td></td></tr><tr><td rowspan=1 colspan=1>c0→c1</td></tr><tr><td rowspan=1 colspan=3>Cluster 1</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 2</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c2→c3</td></tr><tr><td rowspan=1 colspan=3>Cluster 3</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 4</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c4→c6</td><td rowspan=1 colspan=1>c4→c5</td></tr><tr><td rowspan=1 colspan=3>Cluster 5</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 6</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c6→c7</td></tr><tr><td rowspan=1 colspan=3>Cluster 7</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 8</td><td rowspan=1 colspan=1>c8→c12</td><td rowspan=1 colspan=1>c8→c10</td><td rowspan=1 colspan=1>c8→c9</td></tr><tr><td rowspan=1 colspan=3>Cluster 9</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 10</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c10→c11</td></tr><tr><td rowspan=1 colspan=3>Cluster 11</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 12</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c12→c14</td><td rowspan=1 colspan=1>c12→c13</td></tr><tr><td rowspan=1 colspan=3>Cluster 13</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr><tr><td rowspan=1 colspan=3>Cluster 14</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1>c14→c15</td></tr><tr><td rowspan=1 colspan=3>Cluster 15</td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td><td rowspan=1 colspan=1></td></tr></table>

Figure 13. 2D tree multicast.

![](images/9bdf1d31b1b92538bf8f4d739f030f45e3930268d1907adf4736234356b33fad.jpg)  
Figure 14. 2D naive tree reduction.

![](images/09d101ac244269f9ed32a50b0332f9c040852e4a068c7773516451baacfaf299.jpg)  
Figure 15. 2D double-buffered tree reduction.

![](images/b9c0f5089666f06e3c547cde32bac3c63297b57d462352042751a9d409f1375f.jpg)  
