...
Input 3: offline load distribution W of shape $B \times L \times E \colon$ L = length(T [R[0]]) // number of MoE layers   
...
Ws = W.sum(dim = 0) // Ws shape (L × E) if c ≥ r and $\mathrm { d p } [ \ell - 1 ] [ c - r ] > - \infty$ then   
...
$g _ { e l } = P . g p u s ( e , \ell )$ // GPUs with expert copy end for   
for g in gel do end for   
...
${ \bar { T [ } } r ] [ \ell ] = ( B A L { \bar { \ell } } ] / B ) - b a s e$ end for   
end if return x   
end for   
return T
...
Algorithm 3 Balanced Intra-Layer Interleaved Replica ${ \mathrm { A s } } -$   
signment   
Input 1: number of MoE layers L   
Input 2: number of GPUs D   
Input 3: replica count per-layer $R [ 0 . . L - 1 ]$   
Output: assignment matrix A of shape $L \times D ,$ where   
$A [ \ell ] [ g ]$ is the number of experts on GPU g for layer ℓ   
Auxiliary Functions:   
MINCUTOFF $( \mathbf { v } , r )$ : returns the r-th smallest value in   
list v (1-based).   
INTERLEAVESELECT $( \mathcal { T } , k ) \colon$ given an ordered list of   
...
$A [ \ell ] [ g ] = \left\lfloor { \frac { R [ \ell ] } { D } } \right\rfloor$ for $\ell \in [ 0 , L )$ and $g \in [ 0 , D )$   
// total # experts per-GPU across all layers, should keep   
this consistent across all GPUs   
$G [ g ] = \sum _ { \ell = 0 } ^ { L } A [ \ell ] [ g ]$ for $g \in [ 0 , D )$   
for $\ell = \overset { \sim } { 0 }$ to $L - 1$ do   
// only need to handle remainder replicas at each layer   
$r = \dot { R } [ \ell ]$ mod D   
i $\mathrm { ~ r ~ } > 0$ then   
cutoff = MINCUTOFF $( \mathrm { G } , r )$   
// GPUs with # experts < cutoff are always selected   
$M = [ g . . ]$ for all $g \in G$ and $g <$ cutoff   
$q = r - | M |$   
if $q > 0$ then   
// GPUs with tied # experts = cutoff   
$K = s o r t ( [ g . . ] )$ for all $g \in G$ and g = cutoff   
// select among K in evenly-spread manner   
J = INTERLEAVESELECT(K, q) where $J \subseteq K$   
$S = M \cup J$   
else   
$S = M$   
end if   
