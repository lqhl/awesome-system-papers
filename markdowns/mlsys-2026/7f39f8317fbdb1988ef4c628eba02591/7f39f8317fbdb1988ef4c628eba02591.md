...
Notations. Let $\mathbf { G } ^ { t } ~ = ~ ( V ^ { t } , E ^ { t } )$ denote the computational graph of the t-th task in an RL workflow, where $t \in \{ 1 , \ldots , T \}$ , $V ^ { t }$ denotes the set of computational operators, and $E ^ { t }$ denotes the set of tensors shared between operators. The overall computational graph of the RL workflow is then defined as $\mathbf { G } \overset { \cdot } { = } ( \bigcup _ { t = 1 } ^ { T } \breve { V ^ { t } } , \breve { \bigcup } _ { t = 1 } ^ { T } E ^ { t } \cup E ^ { \mathrm { i n t e r } } )$ where $E ^ { \mathrm { i n t e r } }$ denotes the set of edges between tasks.

Let $\mathbf { G } _ { \mathbf { D } } = ( V _ { D } , E _ { D } )$ denote the device topology graph for a heterogeneous environment, where $V _ { D } = \{ d _ { 1 } , \ldots , d _ { N } \}$ are a set of N devices and $E _ { D } \subseteq V _ { D } \times V _ { D }$ are communication channels between devices. Each device d is labeled with computation capability, memory capacity, and HBM bandwidth. Each edge between d and d′ is labeled with the latency and bandwidth.

A partitioning strategy ρ transforms the given G into a new tasklet graph $\mathbf { G } _ { \mathbf { L } } = \left( V _ { L } , E _ { L } \right)$ by first partitioning the operators with intra-model parallelization and then merging them into tasklets. Each new node $l _ { i , j , k } ^ { t } \in V _ { L }$ is a tasklet, $E _ { L }$ denotes the set of tensors transferred between tasklets, and $t , i , j ,$ k represent the indices of a tasklet in RL tasks, data parallelism, pipeline parallelism, and tensor parallelism, respectively. An assignment strategy $\sigma : V _ { L }  V _ { D }$ assigns, for each tasklet $l \in V _ { L }$ , a device $d \in V _ { D }$ . Let $C ( \cdot )$ denote a cost model that estimates cost (i.e., execution time per iteration) of a given workflow, conditioned on the given resource, partitioning strategy, and assignment strategy.
...
Definition 1 (Heterogeneity-Aware RL Training Scheduling Problem). Given a computational graph G for an RL workflow and a device topology graph $\mathbf { \bar { G } } _ { \mathbf { D } } ^ { - }$ for a heterogeneous environment, the heterogeneity-aware RL training scheduling problem is to determine an optimal scheduling strategy, which consists of a partitioning strategy ρ and an assignment strategy σ, such that the execution time of the RL workflow is minimized and resource constraints are satisfied:

$$
\operatorname* { m i n } _ { \rho , \sigma } \quad C ( \rho , \sigma ; { \bf G } , { \bf G } _ { \mathrm { D } } )
$$

$$
\begin{array} { r l } { \mathrm { s . t . } } & { { } | \{ ( i , j , k ) : l _ { i , j , k } ^ { t } \in V _ { L } \} | \leq | V _ { D } | , \forall t \in \{ 1 , \ldots , T \} } \end{array}\tag{C1}
$$

$$
\bigcup _ { d \in V _ { D } } \sigma ^ { - 1 } ( d ) = V _ { L }\tag{C2}
$$

$$
\operatorname* { m a x } _ { l \in \sigma ^ { - 1 } ( d ) } \operatorname { w o r k i n g - m e m } ( l ) + \sum _ { l \in \sigma ^ { - 1 } ( d ) } { \mathrm { m o d e l . m e m o r y } } ( l )
$$

$$
\leq \mathrm { g p u . m e m } ( d ) , \forall d \in V _ { D }\tag{C3}
$$
...
Interpretation. Our framework can be viewed as a coarseto-fine constructive approach that operationalizes the joint optimization over $( \rho , \sigma )$ defined in Section 3.1. Concretely, Levels 1 and 4 instantiate the partitioning strategy $\rho$ (task grouping and intra-model parallelization that produce the tasklet graph $G _ { L } ) _ { \ l }$ , while Levels 2, 3, and 5 instantiate the assignment strategy σ (coarse-to-fine GPU assignments). Accordingly, the procedure constitutes a coarse-to-fine algorithmic instantiation of the RL training scheduling problem.
...
$$
{ \cal C } _ { \mathrm { S y n c P P O } } = { C } ^ { 1 } + \Phi ( \{ C ^ { 2 } , C ^ { 3 } , C ^ { 4 } \} ) + \Phi ( \{ C ^ { 5 } , C ^ { 6 } \} )
$$

where $C ^ { 1 : 6 } ( \cdot )$ denote the cost model of actor generation, reward inference, reference inference, critic inference, critic training, and actor training, respectively. $\Phi ( \cdot )$ aggregates

the costs of tasks without dependencies and is defined as:

$$
\Phi ( \{ C ^ { t } \} ) = \operatorname* { m a x } _ { t } C ^ { t } + ( 1 - \eta ) \left( \sum _ { t } C ^ { t } - \operatorname* { m a x } _ { t } C ^ { t } \right) , \eta \in [ 0 , 1 ] ,
$$
...
$$
C ^ { t } = \left\{ \begin{array} { l l } { \Psi ^ { \mathrm { g e n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { h b m } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } ) , } & { t = 1 , } \\ { \Psi ^ { \mathrm { i n f } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } ) , } & { t \in \{ 2 , 3 , 4 \} , } \\ { \Psi ^ { \mathrm { t r a i n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } , C _ { \mathrm { d p } } ^ { t } , C _ { \mathrm { b u b b l e } } ^ { t } ) , } & { t \in \{ 5 , 6 \} , } \end{array} \right.
$$

where $C _ { \mathrm { c o m p } } ^ { t } ( \cdot )$ estimates a set of per-tasklet or persubgraph computation costs, $C _ { \mathrm { h b m } } ^ { t } ( \cdot )$ estimates a set of per-tasklet or per-subgraph HBM costs (i.e., loading LLMs from HBM to SRAM during decoding), $C _ { \mathrm { { b u b b l e } } } ^ { t } ( \cdot )$ estimate a set of per-tasklet or per-subgraph pipeline-bubble costs, and $C _ { \mathrm { t p } } ^ { t } ( \cdot ) , C _ { \mathrm { p p } } ^ { t } ( \cdot ) , C _ { \mathrm { d p } } ^ { t } ( \cdot )$ estimates a set of pertasklet or per-subgraph TP, PP, and DP computation costs. Ψgen, Ψinf , Ψtrain estimate task-level costs based on the given sets of tasklet- or subgraph-level costs. For brevity, we have omitted some inputs to the above equations, including device and network attributes, and we have also omitted the resharding cost in the end-to-end cost. Details of cost modeling can be found in Appendix B.
...
1: let $T G = \{ t g _ { \xi _ { i } } \}$ be the set of all feasible task groupings for a given RL
workflow, where $\dot { t g } _ { \xi _ { i } }$ denotes a feasible grouping of RL tasks
2: let $G G = \{ t g _ { \xi _ { i } } \ \mapsto \ G G _ { \xi _ { i } } \}$ be the set of all feasible GPU groupings and
$G G _ { \xi _ { i } } = \{ g g _ { \xi _ { i , j } } \}$ i be the set of all feasible GPU groupings for a given tgξ
3: let $C = \{ t g _ { \xi _ { i } } , \overset { \cdot } { g } g _ { \xi _ { i , j } } \mapsto \{ c _ { \xi _ { i , j , k } } \} \}$ be the cost of candidate plans, where
$c _ { \xi _ { i , j , k } }$ denote the k-th cost of candidate plans with task grouping $t g _ { \xi _ { i } }$ and
GPU grouping $g g _ { \xi _ { i , j } }$
4: initialize $T G \gets \emptyset , \overset { \cdot } { G } G \gets \emptyset , C \gets \emptyset$
5: T G ← task grouping(G)
6: for all $t g _ { \xi _ { i } } \in T \tilde { G } \tilde { \mathbf { d o } }$
7: ${ G G } _ { \xi _ { i } } \gets$ gpu grouping( $N , | t g _ { \xi _ { i } } | )$
8: GG $ G G \cup \{ t g _ { \xi _ { i } } \mapsto G G _ { \xi _ { i } } \}$
9: for all $g g _ { \xi _ { i , j } } \in G G _ { \xi _ { i } }$ do
10: $C \gets C \cup \{ t g _ { \xi _ { i } } , g g _ { \xi _ { i , j } } \mapsto \{ \infty \} \}$
11: end for
12: end for
13: $T G _ { 0 } \gets T G$
14: $G G _ { 0 }  G G$
15: for $n = 0 , 1 , . . . , \lceil \log _ { 2 } ( | T G | ) \rceil - 1$ do
16: for all $t g _ { \xi _ { i } } \in T \dot { G } _ { m } \mathbf { \bar { d o } }$
17: let $\begin{array} { r } { b _ { m } = \lfloor \frac { B } { \lceil T G _ { m } \rceil \lceil \log _ { 2 } ( \lvert T G \rvert ) \rceil } \rfloor } \end{array}$ be search budget for task group $t g _ { \xi _ { i } }$
18: $G G _ { \xi _ { i } , 0 } \gets G G _ { m } ( t g _ { \xi _ { i } } )$
19: for $n ^ { ' } = 0 , 1 , \dots , \lceil \log _ { 2 } ( | G G _ { m } ( t g _ { \xi _ { i } } ) | ) \rceil - 1$ do
20: for all $g g _ { \xi _ { i , j } } \in G G _ { \xi _ { i } , n }$ do
21: let $\begin{array} { r } { b _ { m , n } = \lfloor \frac { b _ { m } } { | G G _ { \xi _ { i } , n } | \lceil \log _ { 2 } ( | G G _ { m } ( t g _ { \xi _ { i } } ) | ) \rceil } . } \end{array}$ ⌋ be search budget
for task group $t g _ { \xi _ { i } }$ iand GPU group $9 9 \varepsilon _ { i , j }$
22: for $k = 0 , 1 , \ldots , b _ { m , n } - 1$ do
23: let pk ← genetic algorithm( $t g _ { \xi _ { i } } , g g _ { \xi _ { i , j } } , \mathbf { G } _ { \mathbf { D } } , \ldots . )$ be a plan
generated from lower levels
24: $c _ { \xi _ { i , j , k } } = \mathrm { c o s t . m o d e l } ( p _ { k } , { \bf G _ { D } } , . . . )$
25: $C ( t g _ { \xi _ { i } } , g g _ { \xi _ { i , j } } ) \gets C ( t g _ { \xi _ { i } } , g g _ { \xi _ { i , j } } ) \cup c _ { \xi _ { i , j , k } }$
26: end for
27: end for
28: $G G _ { \xi _ { i } , n + 1 } $ best half( $G G _ { \xi _ { i } , n } , C )$
29: end for
30: $G G _ { m + 1 } ( t g _ { \xi _ { i } } )$ ← best half $( G G _ { m } ( t g _ { \xi _ { i } } ) , C )$
31: 32: end for
$T G _ { m + 1 }$ ← best half $T G _ { m } , C )$
33: end for
```
...
<table><tr><td>Model </td><td>Arch</td><td>Size (GB)</td><td>FP16 Perf. (TFLOPS)</td><td>HBM (GB/s)</td><td>Intra (GB/s)</td></tr><tr><td>A100</td><td>Ampere</td><td>40</td><td>312</td><td>2039</td><td>600</td></tr><tr><td>L40S</td><td>Ada</td><td>48</td><td>366</td><td>864</td><td>64</td></tr><tr><td>L4</td><td>Ada</td><td>24</td><td>121</td><td>300</td><td>64</td></tr></table>
...
We extend the notations in Section 3 for device topology graph. Let $\mathbf { G } _ { \mathbf { D } } = ( V _ { D } , E _ { D }$ , comp, mem, hbm, A, B) denote the device topology graph of heterogeneous environments, where $V _ { D } ~ = ~ \{ d _ { 1 } , \ldots , d _ { N } \}$ are a set of N devices and $E _ { D } = V _ { D } \times V _ { D }$ are communication channels between devices. For each device $d ,$ the computation capability, memory capacity, and HBM bandwidth are $\mathrm { c o m p } _ { d } , \mathrm { m e m } _ { d } , \mathrm { h b m } _ { d } \in \mathbb { R } _ { + }$ Collect them into vectors comp, mem, hbm $\mathbf { \Psi } \in \mathbb { R } _ { + } ^ { N }$ . Each edge $e _ { d , d ^ { \prime } }$ is labeled with latency $\alpha _ { d , d ^ { \prime } }$ and bandwidth $\beta _ { d , d ^ { \prime } }$ , where $\alpha _ { d , d ^ { \prime } } , \beta _ { d , d ^ { \prime } } \in \mathbb { R } _ { + }$ Let A, $\mathbf { B } \in \mathbb { R } _ { + } ^ { N \times N }$ be the corresponding matrices.

We extend the notations in Section 3 for tasklet graph. Let $\mathbf { { G } } _ { \mathbf { { L } } i , j } ^ { \ t } = ( V _ { L i , j } ^ { \ t } , E _ { L i , j } ^ { \ t } )$ denote the subgraph constructed by the set of tasklets of the j-th pipeline stage of the i-th replica in the t-th task. Let $\mathbf { { G } } _ { \mathbf { D } _ { i , j } ^ { \textit { t } } } ^ { \textit { t } } = ( V _ { D _ { i , j } ^ { \textit { t } } } ^ { \textit { t } } , E _ { D _ { i , j } ^ { \textit { t } } } )$ be the subgraph constructed by the set of devices assigned to these tasklets and we have $\sigma ( V _ { L _ { i , j } } ^ { ~ t } ) = V _ { D _ { i , j } } ^ { ~ t }$ . Similarly, let $\mathbf { G _ { L } } _ { j , k } ^ { t } = ( V _ { L _ { j , k } } ^ { t } , E _ { L _ { j , k } } ^ { t } )$ denote the subgraph constructed by the set of tasklets of the k-th tensor shards of the j-th pipeline stage in the t-th task and $\mathbf { { G } } _ { \mathbf { D } { j , k } } ^ { t } = ( V _ { D { j , k } } ^ { \ t } , E _ { D { j , k } } ^ { \top t } )$ denote the assigned devices.

Now we introduce other required notations. Let ${ h } _ { 1 } ^ { t } , { h } _ { 2 } ^ { t } , n { l } ^ { t }$ be the hidden size, the intermediate size, and number of layers of the LLM of the t-th task. Let nltj , $\mathrm { T P } _ { j } ^ { t }$ be number of layers and TP degree in $j \mathrm { - t h }$ pipeline stage of the t-th task. Let $n l _ { j } ^ { t }$ be number of layers in j-th pipeline stage of the t-th task. Let ${ \mathrm { s e q } } _ { \mathrm { i n } }$ and $\operatorname { s e q } _ { \mathrm { o u t } }$ denote the maximum sequence lengths of the input prompts and output responses. Let mbs, nm denote micro-batch size and number of microbatches. Please note that we have preprocessed nm based on the number of responses generated per prompt, the data parallelism degree. Let $B _ { \mathrm { B F 1 6 } } , B _ { \mathrm { F P 3 2 } }$ be the data sizes of the BF16 and FP32 data types. Let ring(·) denote a function that returns the set of all feasible ring graphs of given devices. For brevity, we have omitted the vocabulary and token embeddings in the computation, but they are included in our actual implementation.

## B.2 Modeling component-level computation and communication costs.

Tensor parallelism communication. The communication volume transferred between a pair of neighboring GPUs in $\mathbf { G _ { D } } _ { i , j } ^ { t }$ per all-reduce during TP can be estimated by:

$$
\begin{array} { r l } & { \mathrm { c v } _ { t p } ( t , i , j ) = \displaystyle B _ { \mathrm { B F 1 6 } } \cdot m b s \cdot ( \mathrm { s e q } _ { \mathrm { i n } } + \mathrm { s e q } _ { \mathrm { o u t } } ) \cdot h _ { 1 } ^ { t } } \\ & { \quad \cdot \frac { 2 \cdot ( \mathrm { T P } _ { j } ^ { t } - 1 ) } { \mathrm { T P } _ { j } ^ { t } } } \end{array}
$$

The TP communication cost of forward passes for $\mathbf { G } _ { \mathbf { L } _ { i , j } ^ { t } } ^ { \ t }$ can be estimated by:

$$
\begin{array} { r l } & { C _ { \mathrm { t p } } ( t , i , j ) = 2 \cdot n m \cdot n l _ { j } ^ { t } } \\ & { \qquad \quad \cdot \underset { r \in \mathrm { r i n g } ( { \mathbf { G } } _ { \mathbf { D } _ { i , j } ^ { t } } ) } { \operatorname* { m i n } } \ \underset { e _ { d , d ^ { \prime } } \in r } { \operatorname* { m a x } } ( \alpha _ { d , d ^ { \prime } } + \frac { \mathrm { c v } _ { t p } ( t , i , j ) } { \beta _ { d , d ^ { \prime } } } ) } \end{array}
$$

If recomputation is enabled, the TP communication cost of both forward and backward passes for $\mathbf { G } _ { \mathbf { L } _ { i , j } ^ { t } } ^ { \ t }$ can be

estimated by:

$$
\begin{array} { r l } & { C _ { \mathrm { t p } } ( t , i , j ) = 6 \cdot n m \cdot n l _ { j } ^ { t } } \\ & { \qquad \quad \cdot \underset { r \in \mathrm { r i n g } ( { \mathbf { G } _ { \mathbf { D } _ { i , j } ^ { t } } } ) } { \operatorname* { m i n } } \underset { e _ { d , d ^ { \prime } } \in r } { \operatorname* { m a x } } ( \alpha _ { d , d ^ { \prime } } + \frac { \mathrm { c v } _ { t p } ( t , i , j ) } { \beta _ { d , d ^ { \prime } } } ) } \end{array}
$$

The overall TP communication cost can be estimated by:

$$
C _ { \mathrm { t p } } ^ { t } = \operatorname* { m a x } _ { i , j } C _ { \mathrm { t p } } ( t , i , j )
$$
...
$$
\operatorname { c v } _ { p p } ( t , i , j ) = B _ { \mathrm { B F 1 6 } } \cdot m b s \cdot ( \operatorname { s e q } _ { \mathrm { i n } } + \operatorname { s e q } _ { \mathrm { o u t } } ) \cdot h _ { 1 } ^ { t }
$$

The PP communication cost of forward passes between $\mathbf { G } _ { \mathbf { L } _ { i , j } ^ { t } } ^ { \ t }$ and $\mathbf { G } _ { \mathbf { L } _ { i , j + 1 } ^ { t } } ^ { \ t }$ can be by:

$$
\begin{array} { c } { { C _ { \mathrm { p p } } ( t , i , j ) = n m } } \\ { { \ } } \\ { { \cdot \operatorname* { m i n } _ { \scriptstyle d \in { \bf G } _ { \mathbf { D } _ { i , j } ^ { t } } , d ^ { \prime } \in { \bf G } _ { \mathbf { D } _ { i , j + 1 } ^ { t } } } ( \alpha _ { d , d ^ { \prime } } + \frac { \mathrm { c v } _ { p p } ( t , i , j ) } { \beta _ { d , d ^ { \prime } } } ) } } \end{array}
$$

The PP communication cost of both forward and backward passes between $\mathbf { G } _ { \mathbf { L } _ { i , j } ^ { t } } ^ { \ t }$ and $\mathbf { G } _ { \mathbf { L } _ { i , j + 1 } ^ { t } } ^ { \ t }$ can be estimated by:

$$
\begin{array} { c } { C _ { \mathrm { p p } } ( t , i , j ) = 2 \cdot n m } \\ { \cdot \underbrace { \operatorname* { m i n } } _ { d \in \mathbf { G } _ { \mathbf { D } _ { i , j } ^ { t } } , d ^ { \prime } \in \mathbf { G } _ { \mathbf { D } _ { i , j + 1 } ^ { t } } } ( \alpha _ { d , d ^ { \prime } } + \frac { \mathrm { c v } _ { p p } ( t , i , j ) } { \beta _ { d , d ^ { \prime } } } ) } \end{array}
$$

The overall PP communication cost can be estimated by:

$$
C _ { \mathrm { p p } } ^ { t } = \operatorname* { m a x } _ { i , j } C _ { \mathrm { p p } } ( t , i , j )
$$

Data parallelism communication. The communication volume transferred between a pair of neighboring GPUs in $\mathbf { G } _ { \mathbf { D } _ { j , k } ^ { t } }$ per all-reduce during DP can be estimated by:

$$
\begin{array} { r l } & { \mathrm { c v } _ { d p } ( t , j , k ) = { B } _ { \mathrm { B F 1 6 } } \cdot n l _ { j } ^ { t } \cdot ( 4 \cdot { h _ { 1 } ^ { t } } ^ { 2 } + 3 \cdot { h _ { 1 } ^ { t } } \cdot { h _ { 2 } ^ { t } } ) } \\ & { \qquad \cdot \frac { 2 \cdot \left( \vert { \bf { G } } _ { \mathbf { L } } { _ { j , k } ^ { t } } \vert - 1 \right) } { \vert { \bf { G } } _ { \mathbf { L } } { _ { j , k } ^ { t } } \vert \cdot \mathrm { T P } _ { j } ^ { t } } } \end{array}
$$

The DP communication cost for $\mathbf { G } _ { \mathbf { D } _ { j , k } ^ { t } }$ per iteration can be estimated by:

$$
C _ { \mathrm { d p } } ( t , j , k ) = \operatorname* { m i n } _ { r \in \mathrm { r i n g } ( \mathbf { G } _ { \mathbf { D } _ { j , k } ^ { t } } ) } \operatorname* { m a x } _ { e _ { d , d ^ { \prime } } \in r } ( \alpha _ { d , d ^ { \prime } } + \frac { \mathrm { c v } _ { d p } ( t , j , k ) } { \beta _ { d , d ^ { \prime } } } )
$$

The overall DP communication cost can be estimated by:

$$
C _ { \mathrm { d p } } ^ { t } = \operatorname* { m a x } _ { j , k } C _ { \mathrm { d p } } ( t , j , k )
$$

Computation. The computational cost of a transformer layer per forward pass per sample can be estimated by:

$$
\begin{array} { r l } & { C _ { \mathrm { c o m p } } ^ { \mathrm { q k v o - p r o j } } ( t , i , j , k ) = 2 \cdot 4 \cdot ( \mathrm { s e q } _ { \mathrm { i n } } + \mathrm { s e q } _ { \mathrm { o u t } } ) \cdot { h _ { 1 } ^ { t } } ^ { 2 } } \\ & { C _ { \mathrm { c o m p } } ^ { \mathrm { a t t n } } ( t , i , j , k ) = 2 \cdot 2 \cdot ( \mathrm { s e q } _ { \mathrm { i n } } + \mathrm { s e q } _ { \mathrm { o u t } } ) ^ { 2 } \cdot h _ { 1 } ^ { t } } \\ & { C _ { \mathrm { c o m p } } ^ { \mathrm { m l p } } ( t , i , j , k ) = 2 \cdot 3 \cdot ( \mathrm { s e q } _ { \mathrm { i n } } + \mathrm { s e q } _ { \mathrm { o u t } } ) \cdot h _ { 1 } ^ { t } \cdot h _ { 2 } ^ { t } } \\ & { C _ { \mathrm { c o m p } } ^ { \mathrm { l a y e r } } ( t , i , j , k ) = C _ { \mathrm { c o m p } } ^ { \mathrm { q k v o - p r o j } } + C _ { \mathrm { c o m p } } ^ { \mathrm { a t t n } } + C _ { \mathrm { c o m p } } ^ { \mathrm { m p } } } \end{array}
$$

The computation cost of forward passes for $l _ { i , j , k } ^ { t }$ on device $d = \sigma ( l _ { i , j , k } ^ { t } )$ can be estimated by:

$$
C _ { \mathrm { c o m p } } ( t , i , j , k ) = n m \cdot m b s \cdot n l _ { j } ^ { t } \cdot \frac { C _ { \mathrm { c o m p } } ^ { \mathrm { l a y e r } } } { \mathrm { c o m p } _ { d } \cdot \mathrm { T P } _ { j } ^ { t } }
$$

The computation cost of both forward and backward passes for $l _ { i , j , k } ^ { t }$ on device $d = \sigma ( l _ { i , j , k } ^ { t } )$ can be estimated by:

$$
C _ { \mathrm { c o m p } } ( t , i , j , k ) = 3 \cdot n m \cdot m b s \cdot n l _ { j } ^ { t } \cdot \frac { C _ { \mathrm { c o m p } } ^ { \mathrm { l a y e r } } } { \mathrm { c o m p } _ { d } \cdot \mathrm { T P } _ { j } ^ { t } }
$$

The computation cost for $\mathbf { G } _ { \mathbf { L } _ { i , j } } ^ { \ t }$ can be estimated by:

$$
C _ { \mathrm { c o m p } } ( t , i , j ) = \operatorname* { m a x } _ { k } C _ { \mathrm { c o m p } } ( t , i , j , k )
$$

The overall computation cost can be estimated by:

$$
C _ { \mathrm { c o m p } } ^ { t } = \operatorname* { m a x } _ { i , j } C _ { \mathrm { c o m p } } ( t , i , j )
$$

Please note that here we set ${ \mathrm { s e q } } _ { \mathrm { o u t } }$ to 0 in the estimation for the actor generation task.

Pipeline bubbles. The cost of pipeline bubbles for the i-th replica can be estimated by:

$$
\begin{array} { r } { C _ { \mathrm { b u b b l e } } ( t , i ) = \displaystyle \sum _ { j \neq 0 } } \\ { \frac { C _ { \mathrm { c o m p } } ( t , i , j ) + C _ { \mathrm { t p } } ( t , i , j ) + C _ { \mathrm { p p } } ( t , i , j ) } { n m } } \end{array}
$$

The overall cost of pipeline bubbles can be estimated by:

$$
C _ { \mathrm { b u b b l e } } ^ { t } = \operatorname* { m a x } _ { i } C _ { \mathrm { b u b b l e } } ( t , i )
$$

Decoding (HBM-bandwidth bound). The hbm cost for $l _ { i , j , k } ^ { t }$ on device $d = \sigma ( l _ { i , j , k } ^ { t } )$ can be estimated by:

$$
\begin{array} { r l } & { C _ { \mathrm { h b m } } ( t , i , j , k ) = \mathrm { s e q } _ { \mathrm { o u t } } \cdot n m \cdot m b s } \\ & { \quad \quad \cdot \frac { B _ { \mathrm { B F 1 6 } } \cdot n l _ { j } ^ { t } \cdot ( 4 \cdot h _ { 1 } ^ { t ^ { 2 } } + 3 \cdot h _ { 1 } ^ { t } \cdot h _ { 2 } ^ { t } ) } { d b s _ { d } \cdot \mathrm { h b m } _ { d } \cdot \mathrm { T P } _ { j } ^ { t } } , } \end{array}
$$

where $d b s _ { d }$ is the decoding batch size of the LLM serving engine deployed on device d. The hbm cost for the i-th replica can be estimated by:

$$
C _ { \mathrm { h b m } } ( t , i ) = \operatorname* { m a x } _ { j , k } C _ { \mathrm { h b m } } ( t , i , j , k )
$$

The overall hbm cost can be estimated by:

$$
C _ { \mathrm { h b m } } ^ { t } = \operatorname* { m a x } _ { i } C _ { \mathrm { h b m } } ( t , i )
$$
...
$$
\begin{array} { r l } & { \Psi ^ { \mathrm { g e n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } , C _ { \mathrm { h b m } } ^ { t } ) } \\ & { \qquad = \underset { i } { \operatorname* { m a x } } ( \underset { j } { \operatorname* { m a x } } ( C _ { \mathrm { c o m p } } ( t , i , j ) + C _ { \mathrm { t p } } ( t , i , j ) } \\ & { \qquad + C _ { \mathrm { p p } } ( t , i , j ) + C _ { \mathrm { h b m } } ( t , i , j ) ) ) } \end{array}
$$

Inference. The cost of inference tasks can be estimated by:

$$
\begin{array} { r l } & { \Psi ^ { \mathrm { i n f } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } ) } \\ & { \qquad = \underset { i } { \operatorname* { m a x } } ( \underset { j } { \operatorname* { m a x } } ( C _ { \mathrm { c o m p } } ( t , i , j ) + C _ { \mathrm { t p } } ( t , i , j ) } \\ & { \qquad + C _ { \mathrm { p p } } ( t , i , j ) ) ) } \end{array}
$$

Training. The cost of training tasks can be estimated by:

$$
\begin{array} { r l } & { \Psi ^ { \mathrm { t r a i n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } , C _ { \mathrm { d p } } ^ { t } , C _ { \mathrm { b u b b l e } } ^ { t } ) } \\ & { \qquad = \underset { i } { \operatorname* { m a x } } ( \underset { j } { \operatorname* { m a x } } ( C _ { \mathrm { c o m p } } ( t , i , j ) + C _ { \mathrm { t p } } ( t , i , j ) } \\ & { \qquad + C _ { \mathrm { p p } } ( t , i , j ) ) + C _ { \mathrm { b u b b l e } } ( t , i ) ) + C _ { \mathrm { d p } } ^ { t } } \end{array}
$$

For load balancing strategies, one can tune $\mathrm { s e q } _ { \mathrm { i n } } , m b , n l _ { j } ^ { t }$ during cost modeling.

## B.4 Modeling end-to-end costs.

Let $C ^ { 1 : 6 } ( \cdot )$ denote the cost models of actor generation, reward inference, reference inference, critic inference, critic training, and actor training, respectively, and Φ(·) denotes the cost of the tasks without dependencies which can be defined as

$$
\Phi ( \{ C ^ { t } \} ) = \operatorname* { m a x } _ { t } C ^ { t } + ( 1 - \eta ) \left( \sum _ { t } C ^ { t } - \operatorname* { m a x } _ { t } C ^ { t } \right) , \ :
$$
...
$$
\begin{array} { r } { C ^ { t } = \left\{ \begin{array} { l l } { \Psi ^ { \mathrm { g e n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { h b m } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } ) , } & { t = 1 , } \\ { \Psi ^ { \mathrm { i n f } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } ) , } & { t \in \{ 2 , 3 , 4 \} , } \\ { \Psi ^ { \mathrm { t r a i n } } ( C _ { \mathrm { c o m p } } ^ { t } , C _ { \mathrm { t p } } ^ { t } , C _ { \mathrm { p p } } ^ { t } , C _ { \mathrm { d p } } ^ { t } , } \\ { \quad \ : C _ { \mathrm { b u b b l e } } ^ { t } ) , } & { t \in \{ 5 , 6 \} , } \end{array} \right. } \end{array}
$$
...
$$
\begin{array} { r l } & { C _ { \mathrm { S y n c P P O } } = C ^ { 1 } + \Phi ( \{ C ^ { 2 } , C ^ { 3 } , C ^ { 4 } \} ) + \Phi ( \{ C ^ { 5 } , C ^ { 6 } \} ) } \\ & { \qquad + C _ { \mathrm { r e s h a r d } } } \end{array}
$$

Asynchronous PPO.

$$
\begin{array} { r l } & { C _ { \mathrm { A s y n c P P O } } = \operatorname* { m a x } ( C ^ { 1 } , \Phi ( \{ C ^ { 2 } , C ^ { 3 } , C ^ { 4 } \} ) + } \\ & { \quad \quad \quad \Phi ( \{ C ^ { 5 } , C ^ { 6 } \} ) ) + C _ { \mathrm { s y n c } } } \end{array}
$$

Synchronous GRPO.

$$
C _ { \mathrm { S y n c G R P O } } = C ^ { 1 } + \Phi ( \{ C ^ { 2 } , C ^ { 3 } \} ) + C ^ { 6 } + + C _ { \mathrm { r e s h a r d } }
$$

Asynchronous GRPO.

$$
C _ { \mathrm { A s y n c G R P O } } = \operatorname* { m a x } ( C ^ { 1 } , \Phi ( \{ C ^ { 2 } , C ^ { 3 } \} ) + C ^ { 6 } ) + C _ { \mathrm { s y n c } }
$$

where $C _ { \mathrm { r e s h a r d } }$ are resharding cost required for synchronous RL training and $C _ { \mathrm { s y n c } }$ weight synchronization cost required for asynchronous RL training.