...
Iterative Refinement Process. DLM generation is an iterative refinement over N discrete sampling steps. It gradually transforms a fully masked sequence x1 at time t = 1 (step 0) into a clean sequence x0 at t = 0 (step N ). Here, N serves as a hyperparameter controlling the trade-off between generation speed and sample quality. We denote the sequence at a discrete step k (where k = 0, 1, . . . , N ) as xtk with tk = 1 − kN (so t0=1, tN =0). At each step, approximately Lg/N tokens are finalized on average, where Lg is the target generation length.

This refinement process is modeled probabilistically. Specifically, the transition from a state xt to a subsequent state xs 0 ≤ s < t ≤ 1 is governed by the model pθ’s prediction of the clean sequence x0 given the current noisy sequence xt and a conditioning prompt c, i.e.,

$$
\tag{1}
$$

Following prior formulations (Austin et al., 2021a), the reverse-time transition qs|t(xs | xt) is defined for 0 ≤ s < t ≤ 1 and factorizes across tokens. For each token i, the transition probability is defined as:

$$
\tag{2}
$$

where q0|t is the predictive distribution induced by pθ(x0 | xt, c). This formulation reflects three token-level transitions: preserving an unmasked token, keeping a token masked, or unmasking it to a new token.

Sampling Strategies. To instantiate the probabilistic distribution qs|t into an actual generation procedure, practical deterministic strategies are employed. A common choice is low-confidence remasking, where instead of stochastic sampling, the model greedily selects the tokens to unmask (Nie et al., 2025b). Based on the confidence scores from pθ(x0 | xt, c), the top-m masked tokens with the highest probabilities are revealed, while all others remain masked. In practice, this process is often constrained to operate within blocks of tokens, called block-wise decoding, limiting unmasking to localized regions of the sequence. This prevents unnatural early generation (Nie et al., 2025b) and, more importantly, enables KV caching for finalized blocks. Parallel denoising implicitly assumes independence among simultaneously finalized tokens, an idealized assumption that can degrade generation quality as more tokens are unmasked at once (Liu et al., 2025a).
...
Each xtk is a partially refined sequence; the trajectory Tx records its evolution across steps. Figure 1 (left) visualizes an example. This trajectory serves as the foundation for our consistency-based training below.
...
Let pθ denote the teacher DLM and qϕ(· | x) the student DLM initialized from the teacher’s weights. The teacher uses a bidirectional attention mask as in standard DLMs, whereas the student is trained with a block-wise causal mask, as illustrated in Figure 2. To enable consistency training, we collect trajectories offline by running the teacher on domain-specific prompts. For each prompt x, we denote the token-level trajectory as Tx and the corresponding hiddenstate buffer by Hx ∈ RLg×d; the resulting dataset is D = {(x, yˆ, Tx, Hx)} with ground-truth text yˆ included. The overall procedure is summarized in Algorithm 1.

Algorithm 1 Trajectory Collection for Training CDLM   
1: Input: dataset O of pairs (x, yˆ), generation length   
Lg, block size B, total steps N =Lg, teacher DLM pθ,   
temperature set Sτ , target count n   
2: Output: dataset D of quadruples (x, yˆ, Tx, Hx)   
3: Initialize D ← ∅   
4: repeat   
5: Sample (x, yˆ) ∼ O; build input ids x with the answer   
span masked   
6: for each temperature τ ∈ Sτ do   
7: T ← [ ]; H ← 0 ∈ RLg×d   
8: for blocks b = 1, . . . , ⌈Lg/B⌉ do   
9: for steps s = 1, . . . , B do   
10: Run pθ(x); obtain logits ℓ and last hidden h   
11: Sample y and compute confidence c   
12: Finalize top-1 token yi in the current block;   
write hi → H; append x → T   
13: end for   
14: end for   
15: Append (x, yˆ, T , H) to D   
16: end for   
17: until |processed pairs| ≥ n

Prior work reports that block-wise decoding is effective for instruction-tuned models and attains high quality when the number of steps matches the generation length (Nie et al., 2025b). Accordingly, we adopt block-wise decoding with the number of sampling steps equal to the generation length (N=Lg) and finalize exactly one token per step within the current block (the highest-confidence token yi). This configuration places the teacher at its most performant operating point and yields higher-quality trajectories. For white-box distillation, we store not only Tx but also the states required to reconstruct logits. Concretely, we record the last hidden states at the moments when tokens are finalized. We also augment each prompt with multiple trajectories generated at different temperatures. Additional details on dataset construction are provided in Appendix A.1.
...
1: Input: dataset D = {(x, yˆ, Tx, Hx)}, block size B,   
generation length Lg, weights wcons, wdistill, wdlm, stu  
dent DLM qϕ(· | x) with block-wise causal attention,   
total steps N =Lg   
2: Output: updated student parameters ϕ   
3: repeat   
4: Sample (x, yˆ, Tx, Hx) ∼ D   
5: Sample tstart; set tend ← min N,  tstartB  B   
6: Retrieve states at tstart and tend from Tx   
7: Compute LDistillation (Eq. (4)) using Hx   
8: Compute LConsistency (Eq. (5))   
9: Compute LDLM (Eq. (6)) using ground-truth text yˆ   
10: L ← wdistillLDistillation + wconsLConsistency + wdlmLDLM   
11: Update ϕ with L   
12: until max epochs

Notation Given the dataset D = {(x, ˆy, Tx, Hx)}, we first sample a prompt x and its trajectory Tx, then sample a state y ∈ Tx and its block-completion state y⋆. Here, y⋆ denotes the state obtained by fully unmasking the active block of y (thus y and y⋆ are at most B steps apart). The right panel of Figure 1 visualizes this setup. We denote Uy = {i | yi = [MASK], y⋆i ̸= [MASK]} as the newly unmasked token indices between y and y⋆, and Sy = {i | yi = [MASK], y⋆ = [MASK]} as the still-masked token indices at y⋆.

Distillation Loss We use the student’s predictions at state y together with the teacher’s hidden-state buffer Hx. For each i ∈ Uy, we reconstruct the teacher’s distribution by applying ℓ(T )i = lm head(hx,i) and p(Ti ) = softmax(ℓ(T )i ) to the stored hidden state hx,i. With these teacher distributions, we define the distillation objective as
...
Consistency Loss We compare the student’s predictive distributions at state y and block-completion state y⋆, enforcing agreement on still-masked positions. Concretely, we minimize the following objective:

$$
\tag{5}
$$

Here, qϕ− denotes a stop-gradient target (detached from backpropagation) for stable training (Song et al., 2023), and DKL is the forward KL divergence between two distributions. Intuitively, Eq. (5) aligns the student at a lessinformed state with itself at a more-informed state, encouraging stable multi-step jumps along the decoding trajectory. Since DLMs are trained to predict only [MASK] tokens, restricting the loss to still-masked positions (Sy) avoids ill-defined supervision.

DLM loss We include an auxiliary loss, identical to the masked denoising objective used in standard DLM pretraining (Nie et al., 2025b; Ye et al., 2025), to preserve the model’s mask prediction capability. For a prompt x and its ground-truth response yˆ, we randomly sample a masking ratio t ∼ U[0, 1] and independently mask each token with a probability t to obtain the masked sequence yˆt. The resulting objective is
...
At inference time, the student decodes block-wise under a block-causal mask (Figure 2), reusing the KV cache for the prompt and all previously finalized blocks. Within each block, we apply confidence-thresholded parallel finalization: at each step, among the masked positions of the current block, we reveal every token whose confidence exceeds a threshold τconf, following Fast-dLLM (Wu et al., 2025b). We also adopt early stopping: decoding terminates once an <endoftext> token is produced within the current block, analogous to AR decoding. We intentionally avoid additional heuristics such as inter-block parallelism (Wang et al., 2025), which introduce extra hyperparameters whose optimal values are task- and domain-dependent.
...
![](images/0dbb53e952815c6469d4db9f2f35d7a91cfc8dc00f9f1559557fac5841e7d980.jpg)
...
Target Models and Training Datasets We consider two representative open-source DLMs: Dream-7B-Instruct (Ye et al., 2025) and LLaDA-8B-Instruct (Nie et al., 2025b). We derive 7.5k prompts from Bespoke-Stratos-17k (Labs, 2025), filtering to those with prompt length ≤ 512 tokens. For LLaDA, we further augment the corpus with 7.5k mathstyle prompts from DParallel (Chen et al., 2025), using the same length budget. When generating teacher trajectories, we fix the generation length to Lg=256 and the block size to B=32. Additional details are provided in Appendix A.1.

Training Details We fine-tune all models with LoRA (Hu et al., 2022) applied to both attention and MLP modules, using a block-wise causal mask (Figure 2) with B=32 and Lg=256. End-to-end training takes approximately 8 hours for CDLM–Dream and 16 hours for CDLM–LLaDA on 4× NVIDIA A100 (80GB) GPUs. Additional details are in Appendix A.2.
...
![](images/4c9064917a3b7b72cfad0937cede258f9c9902e224b8f958093ac7062cf731ad.jpg)
...
Throughput. CDLM improves naive DLMs in throughput by 3×–21× and also surpasses equal-size autoregressive (AR) baselines (Figure 3). CDLM–Dream achieves 1.2× and 1.1× higher throughput than its AR baseline on GSM8K-CoT (8-shot) and MBPP-Instruct (0-shot), respectively, while CDLM–LLaDA attains 1.3× and 4.2× higher throughput on GSM8K (4-shot) and HumanEval (0-shot). Throughput here reflects both per-step cost and the number of refinement steps. A CDLM refinement step is more expensive than an AR step due to matrix-matrix multiplications for decoding. However, CDLM can finalize multiple tokens per iteration, reducing total steps and increasing effective throughput. For example, from Table 1, CDLM– Dream produces on average 107.4/44.1 ≈ 2.4 tokens per step on GSM8K-CoT and 74.3/33.2 ≈ 2.2 tokens per step on MBPP-Instruct.
...
Distillation vs. consistency in isolation. With a small auxiliary DLM loss (wdlm=0.01), the distillation-only setting (row 1) acts as a strong anchor: it converges quickly (about
...
![](images/1bd5f9b20d226d77347a793d6f65be56fb8f88eb9fc2daeab06f69d7813b40e1.jpg)
...
![](images/6098c085bf9ba71e54fe82dd297307d5fb2de4464988d3b8c96c889e233ed505.jpg)
...
![](images/be96cebdd61835672880fe53d45c9404f3649f8b60028c8034c78bb759f74708.jpg)  
...
Figure 6. Teacher trajectory and hidden-state buffer. Left: Block-wise decoding trajectory. Right: As each token is finalized, the teacher’s last hidden state at that position is written to a fixed-length buffer of size Lg; arrows indicate write indices across steps (toy example with B=3, Lg=6).
...
Table 7 sweeps the token-level confidence threshold τconf ∈ {0.85, 0.90, 0.95} to characterize conservative-aggressive decoding behavior. Increasing τconf makes finalization more conservative, tokens are only accepted when the model is highly confident, so throughput (TPS) decreases and persample latency increases. Decreasing τconf makes decoding more aggressive and has the opposite effect. This monotonic speed trend holds on both GSM8K-CoT and HumanEval-Instruct. Concretely, on GSM8K-CoT, TPS/latency moves from 42.7/2.5 s at τ =0.95 to 57.7/1.9 s at τ =0.85; on HumanEval-Instruct, from 34.4/2.8 s to 47.3/2.0 s.

Table 7. Ablation of the confidence threshold. Performance under different token-level confidence thresholds τconf for CDLM– Dream on GSM8K-CoT (8-shot) and HumanEval-Instruct (0-shot).  
![](images/08987b5afd0108dbfcecb786e0175116f378c9acecc08c6c3f03d839d12114fc.jpg)

Accuracy shows a similar monotonic trend in τconf. On GSM8K-CoT, scores are essentially flat at 78.8 for τ =0.95 and 0.90, with a slight drop at τ =0.85 (78.4). Similarly, on HumanEval-Instruct, the best score appears at the most conservative setting (51.2 at τ =0.95), 3 points higher than the most aggressive setting (τ =0.85). These results suggest that raising τconf trades speed for quality, but the gain is not linear. In practice, we find τconf=0.90 to be a robust default that balances speed and quality across tasks.

## B.3 Sensitivity Analysis on Inference Block Size

We study the effect of varying the inference-time block size by evaluating CDLM models trained with B=32 under block sizes B ∈ {4, 8, 16, 32, 64}. Figure 8 reports accuracy and throughput (TPS) on representative benchmarks.
...
Figure 8. Impact of inference-time block size. Accuracy and throughput (TPS) across different block sizes B ∈ {4, 8, 16, 32, 64} for CDLM–Dream and CDLM–LLaDA. All models are trained with B=32 and evaluated by varying the inference-time block size.
...
![](images/7eab535e2ea8c61eecc9a3aacd783717d005a8cb03ccfac5f0769213647a9964.jpg)

With memory bandwidth of 2039.0 GB/s, the theoretical ridge point is

![](images/ab04afb854d903f649b3fdb485afa9b78f83c7c693fe5fc64a87be1b2212fba3.jpg)
