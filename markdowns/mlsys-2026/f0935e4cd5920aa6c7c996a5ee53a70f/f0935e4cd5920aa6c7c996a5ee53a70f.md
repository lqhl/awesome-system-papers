...
Speculative decoding (SD) has emerged as a highly effective approach to speed up inference in large language models (LLMs). It has been widely used in real workloads, ranging from chat completion (OpenAI, 2022), question answering (Cobbe et al., 2021; Rein et al., 2023) to coding tasks (Jimenez et al.).
...
Workloads. We evaluate six datasets representing diverse real-world applications: CNN/DailyMail (Hermann et al., 2015; See et al., 2017) (summarization), ShareGPT (sha, 2024) (multi-turn chat), InstructCoder (Li et al., 2024a) (code editting), GSM8K (Cobbe et al., 2021) (grade-school math), and two complex reasoning datasets: AIME22- 24 (AoPS; Project Numina) and GPQA-Main (Rein et al., 2023). We detailed the dataset information in Appendix Sec. A.1.
...
<table><tr><td>Batch Size</td><td>w/o SD</td><td>ngram</td><td>Eagle</td><td>Eagle3</td></tr><tr><td>1</td><td>737±1048</td><td>750±1100</td><td>731±1010</td><td>736±1046</td></tr><tr><td>8</td><td>722±994</td><td>676±810</td><td>709±928</td><td>704±916</td></tr><tr><td>16</td><td>708±932</td><td>675±818</td><td>686±853</td><td>686±853</td></tr><tr><td>32</td><td>674±784</td><td>697±887</td><td>693±857</td><td>706±917</td></tr><tr><td>64</td><td>687±852</td><td>696±911</td><td>709±924</td><td>709±924</td></tr><tr><td>128</td><td>714±930</td><td>739±1050</td><td>709±924</td><td>709±924</td></tr></table>
...
End-to-end speedup. We first present the end-to-end throughput comparison in Fig. 1. Across most configurations, SD improves throughput over the baseline without SD. The improvement is most pronounced with small/medium batch sizes, when the system is memory-bound, so lots of compute can be used for proposing and verification.
...
![](images/a0aed12acf64b2439fe7cd7087f8451c07a988105ee6cc7f02637b871c50c84c.jpg)  
(a) Llama3.1-8B, GSM8K

![](images/b88baa473b68913cdc92fd9b6d3f43e4326f3e67b5d1b150243f2d00422db174.jpg)  
(b) Llama3.1-8B, CNN/Dailymail

![](images/474d62d80c277252c42802c1133cb57fee3a3309cf0f9a8ca712ddc03d77163a.jpg)  
(c) Llama3.1-8B, ShareGPT

![](images/6a332c536f6f8fbb1c92e53dc7b8b8c4bced9365a7b3eb36ff6c3c099beb047c.jpg)  
(d) Llama3.1-8B, InstructCoder

![](images/fef340bc7fb35ade190456530bbfee4d26cd3aca28b260890e6ec4c7dc4dc8c8.jpg)  
(e) Llama3-70B, GSM8K

![](images/9655683dc0559730fd38dc74d405e1b8c0521b08c4307a213a89c63b1d863c31.jpg)  
(f) Llama3-70B, CNN/Dailymail

![](images/36681475e9ccdd9236c504f5901460f5a6fc00aadad3a5c9b7d8ac61adef5d71.jpg)  
(g) Llama3-70B, ShareGPT

![](images/9ad04f6c303290935b9140ec8ea53ec219112b681b36729e000633d93fc3af08.jpg)  
(h) Llama3-70B,

![](images/632e064482c6709c80ed325ca22e295e5c5badadfe1c0556e6810359ec88ce26.jpg)  
(i) Qwen3-8B, GSM8K

![](images/ca5ded9ff1e04f3e118ac7b906873a743756e09f713d427a6bdb001f604c26cf.jpg)  
(j) Qwen3-8B, CNN/Dailymail

![](images/663bf762799becb67ef7b145085ef8545b765f43f538080d98afa969050f3eb3.jpg)  
(k) Qwen3-8B, ShareGPT

![](images/150696287883a09fdc009cae4a113b3c014b23c812bc28e1bc5b5101a94910b8.jpg)  
...
Next, we observe that draft-model–based methods outperform EAGLE on the Llama3-70B model in most settings. As shown in Fig. 7, draft-model–based approaches achieve higher token acceptance rates, while their proposing overhead is comparable to that of EAGLE (see Fig. 4). This behavior is expected: draft models are pretrained on largescale corpora, whereas the EAGLE head is only fine-tuned on a relatively limited dataset, which naturally constrains its acceptance rate.

In contrast, on Qwen3-8B, draft-model–based speculative decoding consistently underperforms EAGLE-3 and, in some cases, even the training-free n-gram method. Specifically, for Llama3-70B, we use Llama3.2-1B as the draft model, whereas for Qwen3-8B we use Qwen3-0.6B. Although acceptance rates can be similar across the 70B and

8B setups on the same dataset—or even higher for the 8B case sometimes—draft-model–based SD is nonetheless noticeably less effective on the smaller model. For example, on GSM8K, the acceptance rate is 75% for the 70B setup and 81% for the 8B setup, yet the overall performance gain is substantially lower for Qwen3-8B.
...
![](images/7a65299ab674c7622b28e3fc11334c15f79205611fbc53dd7e71ab4e1713f7d9.jpg)  
(a) Qwen3-8B, GSM8K

![](images/3e76081b305923476bf016c7b04c19575dcd444daaef589751ad84739d41eaaf.jpg)  
(b) Qwen3-8B, CNN/Dailymail

![](images/789e5127d886cd3eb2929d3078fce6c4dede7d5b6aeda9a8e983b821ff4bca3e.jpg)  
(c) Qwen3-8B, ShareGPT

![](images/af70bd5aa484fedff6b124bc0e00622696759dcdfd66e30ef1008e24acca7b85.jpg)  
(d) Qwen3-8B, InstructCoder

![](images/159c9a0193a05a437c4df48fd379bda2db4d879ae5eee99ed9045dffae11cca5.jpg)  
(e) Llama3-70B, GSM8K

![](images/f7c72bcfbeb39c690fc29ad6b4c62f559fd6303ec37c9ef5aee4cfe5efb053d7.jpg)  
(f) Llama3-70B, CNN/Dailymail

![](images/8489af04aa0978ba54585568f4328f225c9be0eb91cc8dc22bf783d8e455e99d.jpg)  
(g) Llama3-70B, ShareGPT

![](images/9d7a06cc0f634cf08f7c2b41b357530eab61a624e202a39effa69c4e308c891a.jpg)  
(h) Llama3-70B,  
InstructCoder  
...
For GLM-4.5-Air, MTP outperforms n-gram but falls short of the theoretical upper bound, achieving 1.3×–1.8× speedup on GPQA-Main. This gap is likely because the released open-source weights include only the first MTP module, which is trained primarily to predict the first token accurately but reused autoregressively for subsequent ones. Consequently, its position-wise acceptance rate (0.92 → 0.68 → 0.38 on GPQA-Main), accuracy declines sharply across drafted tokens, shrinking the acceptance rate and thereby constraining the achievable speedup.
...
<table><tr><td></td><td>No SD/n-gram</td><td>EAGLE</td><td>EAGLE 3</td></tr><tr><td>Static (GiB)</td><td>14.96</td><td>15.43</td><td>15.75</td></tr><tr><td>Per-token (KiB)</td><td>128</td><td>132</td><td>132</td></tr></table>

(a) Memory breakdown on Llama3.1-8B
<table><tr><td></td><td> No SD/n-gram</td><td>EAGLE</td><td>D-M (1B)</td></tr><tr><td>Static (GiB)</td><td>131.4</td><td>133.3</td><td>133.7</td></tr><tr><td>Per-token (KiB)</td><td>320</td><td>324</td><td>352</td></tr></table>

(b) Memory breakdown on Llama3-70B
<table><tr><td></td><td> No SD/n-gram</td><td>EAGLE3</td><td>D-M (0.6B)</td></tr><tr><td>Static (GiB)</td><td>15.25</td><td>16.00</td><td>16.37</td></tr><tr><td>Per-token (KiB)</td><td>144</td><td>148</td><td>256</td></tr></table>
...
![](images/3c48f61e47fa2927d625586a72c8f3cbd5faaf845fbadb5b649215263632d5fd.jpg)  
(a) Llama3.1-8B

![](images/8aab8f677f584acc96bdfff61e7f0901536686458ee090e32b536145b4308138.jpg)  
(b) Llama3-70B

![](images/99fa5cad0b62aea7991a81013b3072a6259da4f683644e8cf9a7be729611d631.jpg)  
(c) Qwen3-8B  
...
Fig. 5 visualizes the number of tokens generated at each output position for InstructCoder and ShareGPT (notice SD generates at least one token at each position, even if all proposed tokens are rejected). Each row represents a sampled request and each column a generation position, with color indicating the number of tokens produced by SD.
...
Fig. 5 gives a high level feeling of the generation length at each position across requests. n-grams and EAGLE show different patterns. n-grams show high variance, lots of very dark and very light positions, some positions generate a lot, while others only generate one. EAGLE show good average generation length (averge color is darker) and also less variance. But there are variance across all dimentions, which we will describe in more detail below.
...
Since different requests can have varying generation lengths, we sample 50 requests from each generation-length category: requests with generation length <4K, between 4–8K, between 8–13K, and over 13K. In Fig. 6, token positions are partitioned into ten intervals, and the mean generation length is computed and plotted for each interval.

For reasoning workloads, generation length generally increases as generation progresses for both methods, but the growth is substantially steeper for n-gram. As shown in Fig. 6, on GPQA-Main with Qwen-3-8B-Thinking, n-gram rises from roughly 1.6–3.7 generated tokens across the sequence in short outputs (<4K tokens) to 2.7–5 tokens in the longest responses (>13K tokens). EAGLE-3 grows much more gradually from around 2.1–2.5 among the shortest requests and to roughly 2.5–5.7 tokens among the longest requests. These results indicate that long scientific and technical generations accumulate locally repetitive structures, such as recurring variables, equations, and phrases, that ngram can increasingly exploit. On the other hand, EAGLEbased methods remain less sensitive to such surface-level recurrence.
...
We observe substantial variance in per-request generation length in Fig. 7. Even within the same workload, the efficiency of speculative decoding can differ drastically. For example, on InstructCoder with Llama3-70B, EAGLE produces relatively short request-level generation lengths (2.7- 7.4 tokens), whereas n-gram and draft-model-based SD achieve longer lengths with wider spreads (1.1–15.0 tokens and 5.6-18.3 tokens, respectively). In other workloads (e.g.

![](images/9ab8fe06fb710dca1f06d670ae3ab3431576f960b215091178503bc955f517c9.jpg)

![](images/0f29aa103882c4a4421e642913049d8d95c96587fafdeca53a6b7c20a533af62.jpg)  
(a) Requests with outputs shorter than 4K tokens

(b) Requests with outputs longer than 13K tokens  
![](images/713c1f349547182cb14035dea51ab714c6cb934eae4bc34b0dc2f515bc2ec013.jpg)  
(c) Requests with outputs of 4K-8K tokens

![](images/4c4819501e5f1b5022973e9cf2b8d2a59d083a43e313e50d60b6ed593f6ead79.jpg)  
...
![](images/f54f5f5647e1068e9eafc358955206041724bb8696dfb639503325764960112f.jpg)  
(a) GSM8K

![](images/40a33baac09202f4c43921fa30b36e6e0042dee613192a22a14b4a5f9edbf317.jpg)  
(b) CNNDailyMail

![](images/97e76853e6b504ad1152181c3d96d1ef2f746b14b97b028bf7bb153439672a83.jpg)  
(c) ShareGPT

![](images/56acd786f6d7141c2c0999eb2189e247cbd3644c058018da2e73e8930b956719.jpg)  
(d) InstructCoder  
...
ShareGPT and CNN/DailyMail) and target models (e.g. Qwen3-8B), draft-model-based methods similarly shows higher generation lengths than n-gram and EAGLE.
...
print(car1.color)
# Replace the string data type used for car color with an enum
data type.
...
![](images/ee876bfd2647f755ee58a27773997355784862728f5e2128dd2411208e24cfdc.jpg)

![](images/3fae44a50e3f81bc434ac9304fe44f4e72ff8376f2070287d0e7b2b9527731c1.jpg)

![](images/f39f5301a893ee05ff534eba9c31ec2a692a794228edc186388d035e3a2cfeb4.jpg)

![](images/c9829e4545fc3609b5d4ebe9703e31006e5735118780abbfb1f92a271cfb4e76.jpg)  
(c) Qwen3-8B, n-gram-fixed-3

(d) Llama3-70B, n-gram-fixed-3  
![](images/ef9c03db3a6704f358eb8b4a95b91a93e447e2a919de0ca6eebd2eaa4b854cbc.jpg)

![](images/ce2af76e4eecbfe24c694df15c751da2d0177eefe9ae08379bbfbb086529c37a.jpg)

(f) Llama3.1-8B, n-gram-fixed-5 vs EAGLE-3  
![](images/72e287043940ad41f23464bc1619888ac1b56cf36b9f15be08dcf31dd4a50ade.jpg)  
(g) Qwen3-8B, n-gram-fixed-5 vs EAGLE-3

![](images/d227323b8f9046107de67755cdfd3d325dabd8f51d8d41e4ef34f9f492230787.jpg)  
...
![](images/88628ca6b65b5fa8c7cf28108e488ed95714ec70d6d651a6c9471619eab290d8.jpg)  
...
As shown in Fig. 10, on the Instructcoder dataset with a batch size of one and n-gram as the SD method, the oracle setup achieves a speedup of approximately 2.75×, whereas the fixed proposed length configuration attains about 2.1× for the best proposed length of 5. Moreover, the gap generally widens as batch size increases: the fixed-k curves drop much faster, while the oracle speedup degrades more gently. This trend is consistent with our observation in Fig. 4. As batch size grows, verification takes up a larger share of the decoding time, so fixed-k methods would incur larger overhead for verifying draft tokens that are ultimately rejected.

## 8.2 An Adaptive Approach to Achieve Optimal Speedup

Although EAGLE generally outperforms the n-gram method, a closer inspection of acceptance behavior reveals complementary strengths between the two. As illustrated in Fig. 11, the per-position differences in accepted tokens exhibit alternating regions of red (favoring EAGLE) and blue (favoring n-gram), indicating that the two methods k=1\_N-gram k=3\_N-gram k=5\_N-gram Oracle\_N-gram Oracle\_Combine k=1\_Eagle k=3\_Eagle k=5\_Eagle Oracle\_Eagle

![](images/62d77c03ceb64999ec48a962e1ddcce7cf56f7912ef596e0541b8e9f7169e457.jpg)  
(a) InstructCoder

![](images/bca570cd2ee2310d89ca9ad8cf991b36c9f85c394de2366b1d53bca6f89704e9.jpg)  
(b) ShareGPT

![](images/d09cf43289ece8aae9e298bcbe9fb672e969ba3a89057a95cb24b1ac8d859e3b.jpg)  
(c) GSM8K

![](images/61412e09754af328fb23fe79e172d23cbe7e0f2cf9d7d4759f7110db7281e832.jpg)  
(d) CNN/DailyMail  
...
Jimenez, C. E., Yang, J., Wettig, A., Yao, S., Pei, K., Press, O., and Narasimhan, K. R. Swe-bench: Can language models resolve real-world github issues? In The Twelfth International Conference on Learning Representations.
...
Project Numina. AI-MO/aimo-validation-aime. https://huggingface.co/datasets/AI-MO/ aimo-validation-aime.
...
Zeng, A., Lv, X., Zheng, Q., Hou, Z., Chen, B., Xie, C., Wang, C., Yin, D., Zeng, H., Zhang, J., Wang, K., Zhong, L., Liu, M., Lu, R., Cao, S., Zhang, X., Huang, X., Wei,
...
![](images/be8702900fe001c8aa35069aa707caac0c78a8fc1b1c0e2ebdaa16f33cef2901.jpg)  
...
$$
M _ { \mathrm { s t a t i c , G i B } } = { \frac { ( P _ { \mathrm { t a r g e t } } + P _ { \mathrm { d r a f t } } ) \cdot 2 } { 2 ^ { 3 0 } } }
$$

Here, $P _ { \mathrm { t a r g e t } }$ is the target model’s parameter size and $P _ { \mathrm { d r a f t } }$ includes any additional parameters introduced by the speculative decoding method, both in units of Billions.

For EAGLE/EAGLE3, $P _ { \mathrm { d r a f t } }$ refers to the parameters used in speculative decoding. For the EAGLE models we used, $P _ { \mathrm { d r a f t } }$ refers to the parameter size of the autoregressive head, which includes one decoding layer and one fully connected (FC) layer. They share the same embedding layer and language modeling (LM) head with the target model. For the EAGLE3 models we used, $P _ { \mathrm { d r a f t } }$ refers to the parameter size of the entire EAGLE3 model. It includes one decoding layer, the multi-layer feature fusion FC layer, a final normalization layer and its own LM head. In addition, the model checkpoints include two 1-dimensional token-id remapping tables (t2d and d2t) that translate between the target model’s vocabulary and the draft LM head’s vocabulary; they typically contribute negligibly to the parameter memory. When loading model weights in vLLM v0.10.1.1, the t2d tensors are skipped, and we exclude them when counting the parameter size. Similarly, these EAGLE3 models reuse the same embedding layer from the target models.

<table><tr><td>Model</td><td>P</td><td>L</td><td>nkv</td><td> $d _ { \mathrm { h e a d } }$ </td></tr><tr><td>Llama3.1-8B-Instruct</td><td>8.03B</td><td>32</td><td>8</td><td>128</td></tr><tr><td>Llama3-70B-Instruct</td><td>70.55B</td><td>80</td><td>8</td><td>128</td></tr><tr><td>Llama3.2-1B-Instruct</td><td>1.23B</td><td>16</td><td>8</td><td>64</td></tr><tr><td>Qwen3-8B</td><td>8.19B</td><td>36</td><td>8</td><td>128</td></tr><tr><td>Qwen3-0.6B</td><td>0.596B</td><td>28</td><td>8</td><td>128</td></tr><tr><td>EAGLE-LLaMA3.1-Instruct-8B</td><td>0.25B</td><td>1</td><td>8</td><td>128</td></tr><tr><td>EAGLE3-LLaMA3.1-Instruct-8B</td><td>0.425B</td><td>1</td><td>8</td><td>128</td></tr><tr><td>EAGLE-LLaMA3-Instruct-70B</td><td>0.99B</td><td>1</td><td>8</td><td>128</td></tr><tr><td>EAGLE3-Qwen3-8B</td><td>0.40B</td><td>1</td><td>8</td><td>128</td></tr></table>

Table 3. Model specs used for memory calculations. $P$ is the parameter size of the weights used, obtained by counting their parameters; L is the number of hidden layers, $n _ { \mathrm { k v } }$ is the number of Key/Value heads, and $d _ { \mathrm { h e a d } }$ is the head dimension, obtained from the model’s configuration files on Hugging Face.

For Qwen3-0.6B, tie word embeddings defaults to True and its LM head shares the same weight matrix as the embedding layer; thus, we exclude the size of its language modeling (LM) head.

For example, the static memory for Llama3-70B-Instruct with Llama3.2-1B-Instruct as the draft model is (70.55 + $1 . 2 3 ) * 1 0 ^ { 9 } * 2 / 2 ^ { 3 0 } = 1 3 3 . 7 G i B$ . For Llama3.1-8B-Instruct with EAGLE-LLaMA3.1-Instruct-8B, it is $( 8 . 0 3 + 0 . 2 5 ) *$ $1 0 ^ { 9 } * 2 / 2 ^ { 3 0 } = 1 5 . 4 2 G i B$

Per-token KV cache (KiB). To calculate the KV cache size of each generated token:

$$
M _ { \mathrm { K V / t o k e n , K i B } } = \frac { L _ { \mathrm { h } } \cdot 2 \cdot n _ { \mathrm { k v } } \cdot d _ { \mathrm { h e a d } } \cdot 2 } { 2 ^ { 1 0 } } .
$$

$L _ { \mathrm { h } }$ is the number of hidden layers, $n _ { \mathrm { k v } }$ is the number of KV heads (since our models are all Grouped-Query-Attentionbased), $d _ { \mathrm { h e a d } }$ is the head dimension, the first factor 2 is for both Key and Value, and the last factor 2 is bytes/element for FP16. For Llama3-70B-Instruct without speculative decoding or model-free SD methods like n-gram, it is 80 ∗ 2 ∗ 8 ∗ $\bar { 1 2 8 } * 2 / 2 ^ { 1 0 } = \ 3 2 0 K i B$ . Similarly, for Llama3.1- 8B-Instruct, it is $3 2 * 2 * 8 * 1 2 8 * 2 / 2 ^ { 1 0 } = 1 2 8 K i B$

For draft-model-based SD, the total per-token KV cache is the sum over each generated token by the target and draft model. For Llama3-70B-Instruct with Llama3.2-1B-Instruct as the draft model, the per-token KV cache memory size is $8 0 * 2 * 8 * 1 2 8 * 2 / 2 ^ { 1 0 } + 1 6 * 2 * 8 * 6 4 * 2 / 2 ^ { 1 0 } = 3 5 2 K i B .$

Similarly, for EAGLE-based SD, the total per-token KV cache is the sum over each generated token by the target and EAGLE heads. For Llama3-70B-Instruct with Llama3.2-1B-Instruct as the draft model, the per-token KV cache memory size is $8 0 * 2 * 8 * 1 2 8 * 2 / 2 ^ { 1 0 } + 1 * 2 * 8 * 1 2 8 * 2 / 2 ^ { 1 0 }$ $= 3 2 4 K i B$ . The value 1 here means that EAGLE uses an additional transformer block for proposal generation, which effectively adds one extra layer.

## A.3 Acceptance Behaviour

![](images/c628f8c2514993b94208dbd50830c2bac9a7b2e43e9e669f7d79cb03a36e1b00.jpg)  
(a) AIME22-24

![](images/7a21473ccafe5d865631777cda50de1842291b557a6acc939c62eaa82e831126.jpg)  
(b) GPQA-Main
...
<table><tr><td>Dataset</td><td>Method</td><td>Mean</td><td>Median</td><td>Std</td></tr><tr><td>InstructCoder InstructCoder</td><td>n-gram EAGLE</td><td>7.27</td><td>6.57</td><td>4.19</td></tr><tr><td>InstructCoder CNN/DailyMail</td><td>EAGLE-3 EAGLE-3</td><td>4.24 4.18</td><td>4.21 4.06</td><td>1.07 0.99</td></tr><tr><td>CNN/DailyMail</td><td>EAGLE</td><td>3.08</td><td>2.97</td><td>0.61</td></tr><tr><td>CNN/DailyMail</td><td>n-gram</td><td>2.32 2.33</td><td>2.31 1.96</td><td>0.21</td></tr><tr><td>ShareGPT</td><td>EAGLE-3</td><td>3.06</td><td>2.92</td><td>1.08 0.87</td></tr><tr><td>ShareGPT ShareGPT</td><td>EAGLE</td><td>2.82</td><td>2.68</td><td>0.80</td></tr><tr><td>GSM8K</td><td>n-gram EAGLE-3</td><td>2.02 3.02</td><td>1.37</td><td>1.87</td></tr><tr><td>GSM8K</td><td>EAGLE</td><td></td><td>2.92</td><td>0.47</td></tr><tr><td>GSM8K</td><td></td><td>2.56</td><td>2.51</td><td>0.25</td></tr><tr><td></td><td>n-gram</td><td>1.41</td><td>1.28</td><td>0.45</td></tr><tr><td>GPQA_Main</td><td></td><td>4.20</td><td></td><td></td></tr><tr><td></td><td>n-gram</td><td></td><td>3.73</td><td>2.24</td></tr><tr><td>GPQA_Main</td><td>EAGLE-3</td><td>2.78</td><td>2.49</td><td>0.87</td></tr><tr><td>AIME22-24</td><td></td><td></td><td></td><td></td></tr><tr><td>AIME22-24</td><td>n-gram EAGLE-3</td><td>3.53 2.62</td><td>2.96 2.50</td><td>2.29 0.54</td></tr></table>
...
<table><tr><td>Dataset</td><td>Model</td><td>Hardware SD Method</td><td></td><td>Max Output Len. Batch Size</td><td></td></tr><tr><td>CNNDailyMail</td><td>Llama3.1-8B-Instruct 1×H100</td><td></td><td>n-gram,EAGLE,EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>ShareGPT</td><td>Llama3.1-8B-Instruct</td><td>1×H100</td><td>n-gram, EAGLE, EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td></td><td>InstructCoder Llama3.1-8B-Instruct</td><td>1×H100</td><td>n-gram,EAGLE,EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>GSM8K</td><td>Llama3.1-8B-Instruct</td><td>1×H100</td><td>n-gram,EAGLE,EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>CNNDailyMail</td><td>Llama3-70B-Instruct</td><td>4×H100</td><td>n-gram, EAGLE, DM</td><td>8K</td><td>1-128</td></tr><tr><td>ShareGPT</td><td>Llama3-70B-Instruct</td><td>4×H100</td><td>n-gram,EAGLE,DM</td><td>8K</td><td>1-128</td></tr><tr><td></td><td>InstructCoder Llama3-70B-Instruct</td><td>4×H100</td><td>n-gram, EAGLE, DM</td><td>8K</td><td>1-128</td></tr><tr><td>GSM8K</td><td>Llama3-70B-Instruct</td><td>4×H100</td><td>n-gram, EAGLE, DM</td><td>8K</td><td>1-128</td></tr><tr><td>CNNDailyMail</td><td>Qwen3-8B</td><td>1×H100</td><td>n-gram, EAGLE-3,DM (v0.11.1rc1)</td><td>8K</td><td>1-128</td></tr><tr><td>ShareGPT</td><td>Qwen3-8B</td><td>1×H100</td><td>n-gram, EAGLE-3, DM (v0.11.1rc1)</td><td>8K</td><td>1-128</td></tr><tr><td>InstructCoder</td><td>Qwen3-8B</td><td>1×H100</td><td>n-gram, EAGLE-3, DM (v0.11.1rc1)</td><td>8K</td><td>1-128</td></tr><tr><td>GSM8K</td><td>Qwen3-8B</td><td>1×H100</td><td>n-gram,EAGLE-3,DM (v0.11.1rc1)</td><td>8K</td><td>1-128</td></tr><tr><td>CNNDailyMail</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td> n-gram, EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>ShareGPT</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td>n-gram,EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>InstructCoder</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td>n-gram, EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>GSM8K</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td>n-gram, EAGLE-3</td><td>8K</td><td>1-128</td></tr><tr><td>Reasoning workloads</td><td></td><td></td><td></td><td></td><td></td></tr><tr><td>AIME(22--24)</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td>n-gram, EAGLE-3</td><td>32K</td><td></td></tr><tr><td>GPQA-Main</td><td>Qwen3-8B-Thinking</td><td>1×H100</td><td>n-gram,EAGLE-3</td><td>32K</td><td>1-16 1-16</td></tr><tr><td>AIME(22--24)</td><td>GLM-4.5-Air</td><td>4×H100</td><td>n-gram, MTP (v0.11.1rc1)</td><td>32K</td><td>1-4</td></tr><tr><td>GPQA-Main</td><td>GLM-4.5-Air</td><td>4×H100</td><td>n-gram, MTP (v0.11.1rc1)</td><td>32K</td><td>1-4</td></tr></table>

git checkout <branch>

ENV\_DIR=/path/to/envs bash scripts/rebuild\_env.sh conda activate /path/to/envs

## (2) Simulator:

cd simulator/

conda create -n specbench\_simulator python=3.10 -y conda activate specbench\_simulator pip install -r requirements.txt

## B.5 Experiment workflow

(1) vLLM profiling. Set CUDA VISIBLE DEVICES at the top of the target script, then from scripts/:

export SHAREGPT\_PATH=/path/to/sharegpt.json bash run-l3-8b.sh # 1 GPU
...
(2) Simulator. Individual proposers, datasets, prediction methods, and batch sizes can be configured via CLI flags to main.py. Refer to the README on GitHub for more
...
<table><tr><td>Variant</td><td>Target Model</td><td># Draft Tokens</td><td> Draft / Auxiliary Model (Hugging Face)</td></tr><tr><td rowspan="2">EAGLE</td><td>Llama3.1-8B-Instruct</td><td>3</td><td>yuhuili/EAGLE-LLaMA3.1-Instruct-8B</td></tr><tr><td>Llama3-70B-Instruct</td><td>3</td><td>yuhuili/EAGLE-LLaMA3-Instruct-70B</td></tr><tr><td rowspan="2">EAGLE-3</td><td>Llama3.1-8B-Instruct</td><td>3</td><td>yuhuili/EAGLE3-LLaMA3.1-Instruct-8B</td></tr><tr><td>Qwen3-8B</td><td>3</td><td>AngelSlim/Qwen3-8B_eagle3</td></tr><tr><td rowspan="2">Draft-Model-Based</td><td>Llama3-70B-Instruct</td><td>3</td><td> meta-llama/Llama-3.2-1B-Instruct</td></tr><tr><td>Qwen3-8B</td><td>3</td><td>Qwen/Qwen3-0.6B</td></tr></table>
