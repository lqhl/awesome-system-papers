...
- 作者：Patrick H. Coppock, Brian Zhang, Todd C. Mowry, Dimitrios Skarlatos（CMU），Dan Schatzberg（Meta）
...
- 作者：John Alistair Kressel, Hugo Lefeuvre, Pierre Olivier（Manchester, UBC）
...
- 作者：Leon Schuermann, Brad Campbell, Branden Ghena, Philip Levis, Amit Levy, Pat Pannuto（Princeton, UVA, Stanford, UCSD）
...
- 作者：Wonkyo Choe, Rongxiang Wang, Afsara Benazir, Felix Xiaozhu Lin（UVA）
...
- 作者：Saar Amar, Tony Chen, David Chisnall, Ben Laurie, Hugo Lefeuvre, Simon W. Moore, Robert N.M. Watson 等（Apple, Microsoft, Cambridge, SRI International, UBC）
...
- 作者：Charly Castes, Francois Costa, Neelu S. Kalani, Timothy Roscoe, Nate Foster, Thomas Bourgeat, Edouard Bugnion（EPFL, ETH Zurich, Cornell）
...
- 作者：Yuhong Zhong, Daniel S. Berger, Asaf Cidon（Columbia, Microsoft Azure, Microsoft Research, UIUC, MIT）
...
- 作者：Seung-seob Lee, Jachym Putta, Anurag Khandelwal（Yale），Ziming Mao（UC Berkeley）
...
- 作者：Yueyang Pan, Yash Lala, Sanidhya Kashyap（EPFL），Seung-seob Lee, Anurag Khandelwal（Yale）
...
- 作者：Artem Y. Polyakov, Gal Shalom, Liran Liss（NVIDIA）
...
- 作者：Junliang Hu, Zhisheng Hu, Ming-Chang Yang（CUHK），Chun-Feng Wu（NYCU）
...
- 作者：Borui Wan, Gaohong Liu, Zuquan Song 等（ByteDance Seed, HKU）
...
- 作者：Foteini Strati, Zhendong Zhang, Ana Klimovic（ETH Zurich），Song Han（MIT），等 25+ 机构
...
- 作者：Chenyu Jiang, Zhenkun Cai, Ye Tian, Chuan Wu（HKU, AWS）
...
- 作者：Yunchi Lu, Youshan Miao, Peng Huang（U Michigan, Microsoft Research, Northeastern）
...
- 作者：Yangtao Deng, Lei Zhang, Qinlong Wang 等（CUHK, ByteDance, Harvard）
...
- 作者：Yigong Hu, Zeyin Zhang, Yicheng Liu 等（Boston U, Johns Hopkins, U Michigan, UCLA, U Washington）
...
- 作者：Chenxiao Liu, Zhenting Zhu, Harry Xu 等（UCAS, UCLA, UC Berkeley, PKU, Tsinghua）
...
- 作者：Yuzhuo Jing, Yuqi Mai, Peter M. Chen, Peng Huang（U Michigan）
...
- 作者：Suhas Jayaram Subramanya, Don Kurian Dennis（Microsoft, Meta），Virginia Smith, Gregory R. Ganger（CMU）
...
- 作者：Le Chen, Dahu Feng, Haibo Chen（SJTU, Tsinghua, SenseTime）
...
- 作者：Yifan Yu, Yu Gan, Fan Lai, Hank Levy 等（UIUC, Google）
...
- 作者：Kuntai Du, Bowen Wang, Chen Zhang 等（U Chicago, Tsinghua, UC Berkeley, TensorMesh）
...
- 作者：In Gim, Zhiyao Ma, Seung-seob Lee, Lin Zhong（Yale）
...
- 作者：Yanqi Zhang, Yuwei Hu, John C.S. Lui, Haibo Chen（Huawei, CUHK, SJTU）
...
- 作者：Chen Zhang, Kuntai Du, Ion Stoica, Joseph Gonzalez（Tsinghua, UC Berkeley, TensorMesh）
...
- 作者：Tal Zussman, Asaf Cidon 等（Columbia, IBM Research）
...
- 作者：Chuandong Li, Ran Yi, Jing Liu 等（PKU, Microsoft Research, Igalia, Michigan Tech）
...
- 作者：Yanbo Zhou, Erci Xu, Steven Swanson（UCSD, SJTU, Samsung Semiconductor）
...
- 作者：Franco Solleza, Malte Schwarzkopf, Stan Zdonik 等（Brown, UW, Northwestern, Intel, MIT）
...
- 作者：Florian Suri-Payer, Natacha Crooks, Lorenzo Alvisi 等（Cornell, UC Berkeley, UCSD）
...
- 作者：Jinkun Geng, Shuai Mu, Balaji Prabhakar（Stanford, Stony Brook, NYU）
...
- 作者：Juncheol Ye, Seungkook Lee, Dongsu Han 等（KAIST, Chung-Ang）
...
- 作者：Siddhant Ray, Rui Pan, Junchen Jiang 等（U Chicago, Princeton, Microsoft）
...
- 作者：Zhengding Hu, Vibha Murthy, Yufei Ding, Yuke Wang 等（UCSD, Rice）
...
- 作者：Benjamin Ramhorst, Gustavo Alonso 等（ETH Zurich, AMD Research）
...
- 作者：Chenyuan Yang, Zijie Zhao, Lingming Zhang（UIUC, ZJU, SJTU）
...
- 作者：Zhiyong Wu, Jie Liang, Jingzhou Fu, Yu Jiang（Tsinghua, Beihang）
...
- 作者：Kayvan Memarian, Ben Simner, Peter Sewell 等（Cambridge）
...
- 作者：Tao Lyu, Mathias Payer, Sanidhya Kashyap 等（EPFL, Waterloo）
...
- 作者：Yage Hu, Wen Zhang, Wenwen Wang（U Georgia）
...
- 作者：Xiangdong Chen, Zhaofeng Li, Anton Burtsev 等（U Utah, Palo Alto Networks）
...
- 作者：Zihao Zhang, Ti Zhou, Shuai Mu（Stony Brook, Iowa State）
...
- 作者：Vivien Rindisbacher, Evan Johnson, Deian Stefan, Ranjit Jhala 等（UCSD）
...
- 作者：Eli Baum, Sam Buxbaum, John Liagouris 等（Boston U, UT Austin）
...
- 作者：Louis-Henri Merino, Bryan Ford 等（EPFL, King's College London, Boston U, Harvard）
...
- 作者：Ziyue Qiu, Hojin Park, Gregory R. Ganger, George Amvrosiadis（CMU, Uber）
...
- 作者：Bang Di, Yun Xu, Kaijie Guo 等（阿里巴巴）
...
- 作者：Yuxuan Zhang, Sebastian Angel（UPenn）
...
- 作者：Jiahao Li, Kang Chen, Feng Wu 等（USTC, Baidu）
...
- 作者：Tom Kuchler, Ana Klimovic 等（ETH Zurich, MPI-SWS）
...
- 作者：Nicolaas Kaashoek, Amit Levy, Wyatt Lloyd（Princeton, Cornell, Sentient Foundation）
...
- 作者：Shaobo Li, Yirui Eric Zhou, Jian Huang（UIUC）
...
- 作者：Xingda Wei, Zhuobin Huang, Haibo Chen（SJTU, NUS）
...
- 作者：Hongtao Xie, Boxin Zhang, Yongwei Wu, Mingxing Zhang（Tsinghua, Approaching.AI）
...
- 作者：Yuxing Xiang, Xue Li, Xin Jin, Jingren Zhou（PKU, Alibaba）
...
- 作者：Yue Guan, Xinwei Qiang, Yuke Wang, Yufei Ding, Adnan Aziz（UCSD, Meta, Rice, George Mason）
...
- 作者：Jingkai He, Dong Du, Yubin Xia, Haibo Chen（上海交通大学、华为）
...
- 作者：Junyang Zhang, Yuval Tamir, Diyu Zhou 等（北京大学、蚂蚁集团、UCLA、CertiK）
...
- 作者：Youmin Chen, Jiwu Shu, Yanyan Shen（上海交通大学、清华大学）
