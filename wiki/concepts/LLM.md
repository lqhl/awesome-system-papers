...
- [[KernelEvolve-ISCA26]] — 将模型生成、异构硬件知识、多粒度性能剖析和图搜索组合成 Meta 生产内核服务；跨平台测试和生产案例很强，但“100% 正确”仅指论文列举配置全部通过测试。
...
- [[DeepFund-arXiv25]]、[[Market-Bench-arXiv25]]、[[BacktestBench-KDD26]]、[[AlphaForgeBench-KDD26]]、[[AlphaEval-KDD26]] 与 [[Yu-FormulaicAlphaDiscovery-arXiv26]] — 分别从实时投资、参考回测、端到端回测、策略代码生成、多维筛选和跨方法综述说明，模型输出可执行、可重放或表面合理都不等于经济有效；LLM 能扩大候选生成，却不能替代独立的金融适应度验证。
