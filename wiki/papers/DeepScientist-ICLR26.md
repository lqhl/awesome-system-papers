...
- **观察 1：frontier idea 极度稀疏，验证与过滤比 idea 生成更稀缺。** 三任务共生成 4,879 个 idea，实施 1,108 个，最终仅 21 个形成 progress finding；UCB 筛选后的成功率约 1–3%，每任务随机抽 100 个 idea 的 ablation 成功率接近 0（§4.3，Fig. 4）。
  - **依赖假设**：[[LLM|LLM]] reviewer 对 utility、quality、exploration value 的 0–100 估值与真实实验价值正相关。
...
最薄弱的跳步有两个。第一，作者把 benchmark metric 上的新 SOTA 解释为「scientific value」，但 1.9% throughput 优化也可能是局部 engineering；排除与 [[PagedAttention|PagedAttention]] 等已有机制组合是作者的价值判断，不是实验能证明的科学/工程边界。第二，从一次 one-week resource sweep 推出 discovery scaling law 过强；曲线既无随机种子，也无法区分更多并行 trial 和共享 memory 的协同收益。
