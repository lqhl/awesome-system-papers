...
> **一句话总结**：在 129 个合成 symbolic-regression 问题中，直接把 [[LLM]] 放进「数据分析—方程执行—反馈改写」长程闭环，比只让 LLM 充当 equation proposer 更有效；SR-Scientist 以 GPT-OSS-120B 将整体 Acc$_{0.01}$ 从 LLM-SR 的 28.16% 提至 63.57%，但该结论依赖强 BFGS evaluator、每题 1,000 次 LLM call 和已知生成方程的合成 benchmark，尚不能等价为开放科学发现（§4.1–4.3，Table 1）。
...
- **观察 1：data analysis 是有效搜索动作，而不只是结果解释。** GPT-OSS-120B 移除 data-analyzer tool 后，Acc$_{0.01}$ 从 63.57% 降至 35.66%；Qwen3-Coder-480B 从 49.09% 降至 41.08%（§4.3，Table 3）。
...
- **观察 2：固定总调用预算时，单次 trajectory 需要足够深，但越长不一定越好。** 将 maximum turns 从 10 增至 25，三个 backbone 的 Acc$_{0.01}$ 均上升；继续到 30 turns 则停滞或下降，GPT-OSS-120B 约从 63.5% 回落到 60.5%（§4.3，Fig. 5）。
...
- **观察 3：只保留 top-performing equations 的轻量 memory 能跨 iteration 积累进展。** 移除 experience buffer 后，Qwen3-Coder-480B 的 Acc$_{0.01}$ 从 49.09% 降至 35.66%，GLM-4.5-Air 从 48.32% 降至 37.21%；随机采样历史方程也劣于 top-k（§4.3，Table 3）。
...
- **主结果**：LSR-Synth 共 129 题，覆盖 chemistry 36、biology 24、physics 44、materials 25；在每题约 1,000 次 LLM call、三次重复下，GPT-OSS-120B 的 SR-Scientist 达到 Acc$_{0.01}$ 63.57% / Acc$_{0.001}$ 49.35%，对比同 backbone 的 LLM-SR 28.16% / 11.37%（§4.1–4.2，Table 1）。
- **跨 backbone**：Qwen3-Coder-480B、GLM-4.5-Air、GPT-OSS-20B 的 Acc$_{0.01}$ 分别为 49.09%、48.32%、42.64%，对应 LLM-SR 为 41.08%、35.92%、33.33%；说明收益不只来自单一模型，但幅度从 8.01 到 12.40 个百分点不等（§4.2，Table 1）。
- **RL 收益**：Qwen3-Coder-30B-A3B 经 RL 后，Acc$_{0.01}$ 从 32.30% 提至 40.92%，Acc$_{0.001}$ 从 16.02% 提至 20.69%；训练数据同样由合成 equation family 构造，跨真实域迁移仍未知（§4.2，Table 1）。
- **组件消融**：GPT-OSS-120B 去掉 T1、experience、top-k 后 Acc$_{0.01}$ 分别为 35.66%、57.36%、58.14%，完整系统为 63.57%；对 Qwen3-Coder-480B，随机历史采样甚至降至 26.36%（§4.3，Table 3）。
...
- **噪声边界**：训练数据加入 Gaussian noise 后所有方法显著下降；在 $\sigma=0.01$ 时最佳模型从无噪声约 63.6% 降至约 18% Acc$_{0.01}$，到 $\sigma=0.1$ 仅约 10%–13%，虽仍领先 baseline，但绝对可靠性很低（§4.2，Fig. 3）。
- **OOD**：Qwen3-Coder-480B 下总体 OOD Acc$_{0.01}$ 约 47%，略低于 ID 的 49.09% 且高于 LLM-SR；这里的 OOD 是同一生成方程取更高 temperature/time 区间，不是跨实验装置或跨机制分布（§4.2，Fig. 2；§C.1）。

## Claim–Evidence Map

| Claim | Evidence | Evaluation boundary | Confidence |
|---|---|---|---|
| agentic analysis/evaluation loop 显著优于同 backbone 的 LLM-SR | GPT-OSS-120B：63.57% vs 28.16% Acc$_{0.01}$（§4.2，Table 1） | 129 个 LSR-Synth 合成问题；每题约 1,000 LLM calls；最差 5% predictions 被丢弃 | strong |
...
| RL 可提升 agentic equation discovery | Qwen3-Coder-30B Acc$_{0.01}$ 32.30%→40.92%（§4.2，Table 1） | 1,024 个合成训练题；32×H200；同类型四学科 benchmark | medium |
| 系统具备 robust、generalizable scientific discovery 能力 | OOD 仍领先，但 $\sigma=0.01$ 后最佳 Acc$_{0.01}$ 约降至 18%，symbolic accuracy 最高 7.75%（§4.2，Fig. 2–3，Table 2） | OOD 仍来自同一方程；Gaussian synthetic noise；无真实 wet-lab/observational data | weak |
