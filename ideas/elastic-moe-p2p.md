...
[[17e62166fc8586dfa4d1bc0e1742c08b|CRAFT]] 直接占据了"fine-grained per-layer expert replica allocation with cost model"的叙事位：
...
| 问题 | 已有方案 | 状态 |
|---|---|---|
| MoE all-to-all 通信效率 | DeepEP（NVLink+IBGDA）、[[2510.27656v1\|pplx-garden/TransferEngine]]（P2P RDMA） | 成熟 |
...
| **静态 expert replica 分配** | **[[17e62166fc8586dfa4d1bc0e1742c08b\|CRAFT]]（MLSys'26）**：MCKP-based per-layer replica budget，相比 EPLB 副本数减少 7-8x | **新，已解决** |
| **Prefill 单节点 LB** | [[16200_Libra_Effective_yet_Effi\|Libra]]（ICLR'26）：speculative gating + Two-Stage Execution，8×H200 上 +19.2% 吞吐 | 已解决 |
| LB 搬运代价建模 | [[3769695.3771675\|Latency-Optimal LB]]（INET4AI'25）：migration cost 感知 heuristic，搬运量 -57% | workshop 级 |
| 单机 expert offloading | [[3731569.3764843\|KTransformers]]、MoE-Infinity、Pre-gated MoE | 活跃研究 |
| 实例级弹性扩缩 | [[osdi25-zhang-dingyan\|BLITZSCALE]]（OSDI'25）：layer-granularity live scaling | 已解决 |
| Token 级自动扩缩 | [[3731569.3764815\|Aegaeon]]（SOSP'25）：per-token slack budget，97% 扩缩延迟削减 | 已解决 |
...
**为什么必须 disaggregated**：non-disaggregated 模式下，同一 GPU 的同一次 MoE forward 混合了 prefill 和 decode tokens。Expert placement 是 GPU 级状态，无法让 prefill 走一套 placement、decode 走另一套。这不是额外限制——SGLang、DistServe、[[202cb962ac59075b964b07152d234b70|Beyond the Buzz]] 都论证了 PD disaggregation 在大规模 MoE 上是当前工业趋势。
...
3. **去中心化 Controller**：每节点运行一个 Controller，通过 gossip 同步 placement 状态，避免中心化 orchestrator 在 >64 GPU 时的瓶颈（借鉴 [[f4b9ec30ad9f68f89b29639786cb62ef|Matrix]] 的 P2P 编排思想）

### 3.2 四个核心机制

#### 机制 1：自强化信号的负载感知（Self-Reinforcing Load Monitor）

**方法论模式**：借鉴 [[d82c8d1619ad8176d665453cfb2e55f0|BLASST]] 的"重用计算中已有的状态"范式——零额外开销。
...
**错误预算触发器（借鉴 [[1f0e3dad99908345f7439f8ffabdffc4|HELIOS]] 的 CBC 模式）**：维护 `imbalance_breach_counter`，当连续 B 个 step 的 imbalance > θ 时触发决策，而非每个 step 都决策。这把决策频率从 per-step（ms 级）降到 event-driven（秒级），避免在 decode 关键路径上做昂贵决策。

#### 机制 2：Expert 稳定性分类（Offline-Characterized Stability Profile）

**方法论模式**：借鉴 [[76dc611d6ebaafc66cc0879c71b5db5c|FlexiCache]] 的"model-inherent property characterization"——把模型的内禀特征转化为部署时的决策资产。
...
**方法论模式**：借鉴 [[osdi25-mohoney|Quake]] 的"cost-model-driven incremental maintenance + estimate-verify-commit"——每次调整都有 monotone improvement 保证。
...
- 成本建模（吸收 [[3769695.3771675|INET4AI]] 的 insight）：
  ```
...
**借鉴 [[f4b9ec30ad9f68f89b29639786cb62ef|Matrix]] 的 stateless + message-passing 设计**：Controller 本身无状态，状态完全由 gossip 消息承载。
...
[[17e62166fc8586dfa4d1bc0e1742c08b|CRAFT]] 是与 ElasticMoE 最接近的工作，必须清晰差异化。
...
**Libra 的 speculative gating 能否迁移到 decode？** 预测原语（用 $h_\ell$ 预测 $g_{\ell+1}$）本身 workload-agnostic，准确性应可迁移。但 Libra 把预测用于**每层重规划**，而 decode 关键路径无法承受这种开销。**ElasticMoE 可以选择性吸收 speculative gating**：用于在 error-budget breach 后**加速决策的 estimate 阶段**——而非替代 event-driven 架构。这是一个可以单独 ablation 的可选增强。
...
[[2510.27656v1|pplx-garden]] 是通信基础设施，不做 placement 决策。**ElasticMoE 构建在 pplx-garden 之上**，关系类似 Spark 构建在 HDFS 之上。
...
|---|---|---|
| [[17e62166fc8586dfa4d1bc0e1742c08b\|CRAFT]] (MLSys'26) | Per-layer replica budget（static） | Per-expert elasticity（dynamic + multiple ops） |
| [[76dc611d6ebaafc66cc0879c71b5db5c\|FlexiCache]] (MLSys'26) | Head stability 作为 model-inherent property | Expert stability classification |
| [[fc490ca45c00b1249bbe3554a4fdf6fb\|MorphServe]] (MLSys'26) | 离线优先级 + 运行时查表 | Stability class 指导运行时决策 |
| [[d82c8d1619ad8176d665453cfb2e55f0\|BLASST]] (MLSys'26) | 重用已有计算状态做决策 | 复用 pplx-garden routing metadata |
| [[osdi25-mohoney\|Quake]] (OSDI'25) | Cost-model + estimate-verify-commit | Expert placement adjustment 协议 |
| [[osdi25-zhang-dingyan\|BLITZSCALE]] (OSDI'25) | Layer-granular live scaling | Expert-granular live scaling |
| [[3769695.3771675\|Latency-Opt LB]] (INET4AI) | 搬运代价建模 | P2P RDMA 吸收并压低搬运代价 |
| [[f4b9ec30ad9f68f89b29639786cb62ef\|Matrix]] (MLSys'26) | P2P 去中心化编排 | 去中心化 Controller + gossip |
