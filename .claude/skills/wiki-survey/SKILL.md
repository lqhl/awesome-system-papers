...
按共享契约列出反复概念、专名与缩写，为每个术语选定全页唯一表达。综述通常跨多篇论文：若重复专业术语超过 5 种，在第一张矩阵之前增加简短的「阅读提示」；否则在正文首次出现处直接解释。
...
2. **确定性规则**：运行 `uv run python .claude/skills/wiki-lint/lint.py --language-only {OUT_PATH}`，并复查表格内 wikilink 的 `\|`、frontmatter 和核心成员计数。语言 lint 是保守检查；`language_warnings=0` 不能替代共享契约的语义复核。
...
- **中文优先**：综述正文、分类名、表头、表格单元格和普通概念必须通过共享写作契约。
